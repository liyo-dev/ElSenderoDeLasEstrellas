...
# ---------------------------------------------------------------------------------------------
# INC-355/356 (21 sep): el duelo, mas tranquilo
# ---------------------------------------------------------------------------------------------

VFX_BOLA_DE_FUEGO = "eccbc655050af0b4f81d8db39f84a58e"   # FX_Fireball, el del contraataque aereo
VFX_DESTELLO_MANO = "895c6d094b6b213418cddcfb520298e9"
VFX_EXPLOSION = "67a684e320da6e7439421a07e3fa265c"
VFX_ESTRELLA = "98c4704d0fd7211449bcf5c451095a60"        # Star hit


def _nota(b):
    return b.get("note") or ""


def _uno(fase, pred, que):
    hits = [k for k, b in enumerate(fase.beats) if pred(b)]
    if len(hits) != 1:
        raise RuntimeError(f"pase_duelo_fluido: esperaba 1 '{que}' en {fase.nombre}, hay {len(hits)}")
    return hits[0]


def _plano_con(fase, trozo):
    return _uno(fase, lambda b: b.tipo == "ShotBeat" and trozo in _nota(b), trozo)


def pase_duelo_fluido(fases):
    """
    Cuatro cosas que pidio Raul el 21 sep, mirando la grabacion 11 en Play:

    1. «Cuando dice "ni siquiera puedes salvarte tu" hay varios planos que no me gustan: quiero
       quitarlos y unirlo directamente con la contestacion del Archimago.»
    2. «Cuando el Archimago lanza la bola de fuego, el Mago Oscuro hace la animacion de dano antes
       de que le de.»
    3. «El plano de la foto lo quiero quitar porque no se ve nada» (4:00, cesped y casas al fondo:
       un plano general del Mago Oscuro que salta siete metros y se sale por arriba).
    4. «Cuando el Mago Oscuro ataca con el escudo puesto se queda un rato asi (en guardia). Yo le
       romperia el escudo y le tiraria al suelo: hacia arriba, en parabola, con la animacion de
       falling. Se recupera, se pone de pie, y ya enfocamos al Mago Oscuro para el final.»

    Y por encima de las cuatro: «donde haya muchos cambios de camara la sensacion es de saltos;
    debe verse todo tranquilito y fluido». Aqui eso es quitar cortes que no aportan y dejar que
    un plano VIVO siga la accion en vez de trocearla.
    """
    duelo = por_nombre(fases, "7 - El duelo")
    final = por_nombre(fases, "8 - El ultimo hechizo")
    D = duelo.beats
    ...
    # ── 1. «Ni siquiera puedes salvarte tu» → la respuesta, sin nada en medio ────────────────
    #
    # Entre las dos frases habia cuatro planos, la carga de su esfera, una camara lenta, el
    # Archimago levantandose y el Mago Oscuro bajando la mano. Fuera todo. Lo unico que no se
    # puede quitar es que cada uno vuelva a su marca (el hechizo que viene apunta a M_Duelo3_Mago
    # y el despegue sale de M_Duelo3_Oscuro), asi que se recolocan EN EL MISMO CORTE que lleva a
    # la respuesta: las dos colocaciones van primero dentro del Parallel y el plano se resuelve
    # ya con ellos en su sitio.
    i_mago = _uno(duelo, lambda b: b.tipo == "SayBeat" and b.get("textKey") == s("PROLOGO_DUELO_MAGO_2"), "MAGO_2")
    i_resp = _plano_con(duelo, "De pie otra vez")
    medio = D[i_mago + 1:i_resp]
    escalas = [b for b in medio if b.tipo == "TimeScaleBeat"]
    assert len(escalas) == 2, "la camara lenta tenia que ir por parejas"
    vuelta = escalas[1].get("timeScale") or escalas[1].get("scale") or escalas[1].get("value")
    assert vuelta is None or vuelta.startswith("1"), f"la camara lenta no volvia a 1: {vuelta}"
    conservar = [b for b in medio
                 if (b.tipo == "GestureBeat" and b.get("actorId") == s("NPC_Archimago")
                     and b.get("gesture") == s("Idle_Battle_NoWeapon")
                     and "Se levanta" not in _nota(b))
                 or (b.tipo == "EmotionBeat" and b.get("actorId") == s("NPC_Archimago")
                     and medio.index(b) > len(medio) - 4)]
    respuesta = D[i_resp]
    respuesta.set("note", s("CORTE directo desde «Ni siquiera puedes salvarte tu» a su respuesta. "
                            "De pie, y la camara mas baja que el."))
    recoloca = a_la_vez("Cada uno a su marca en el mismo corte que lleva a la respuesta: primero "
                        "se colocan, despues se resuelve el plano, y no se ve ningun salto.", [
        colocar("NPC_MagoOscuro", "M_Duelo3_Oscuro", haciaActor="NPC_Archimago"),
        colocar("NPC_Archimago", "M_Duelo3_Mago", haciaActor="NPC_MagoOscuro"),
        respuesta,
    ])
    D[i_mago + 1:i_resp + 1] = [recoloca] + conservar

    # ── 2. La bola de fuego del asalto 2: que llegue ANTES del dano ─────────────────────────
    #
    # Eran dos VFX clavados (un destello en su mano y un «charge slash» en el pecho del otro) y
    # el TakeDamage detras, todo en el mismo fotograma: no viajaba nada y el golpe se veia antes
    # que el hechizo. Ahora es un SpellBeat de verdad: sale de su mano, cruza el plano, revienta,
    # y SOLO ENTONCES encaja el golpe.
    i_rec = _plano_con(duelo, "CORTE al que recibe")
    i_cara = _plano_con(duelo, "Primera vez en todo el prologo que el Archimago ataca")
    i_dano = _uno(duelo, lambda b: b.tipo == "GestureBeat" and "Le da de lleno" in _nota(b), "Le da de lleno")
    fuera = [b for b in D[i_cara + 1:i_dano]
             if (b.tipo in ("SfxBeat", "ShakeBeat"))
             or (b.tipo == "VfxBeat" and VFX_ESTRELLA not in (b.get("vfxPrefab") or ""))]
    for b in fuera:
        D.remove(b)
    i_rec = _plano_con(duelo, "CORTE al que recibe")
    D.insert(i_rec + 1, hechizo(
        "NPC_Archimago", "Su bola de fuego: sale de la mano, cruza el plano y revienta en el. El "
        "golpe va DETRAS, cuando ha llegado.", objetivo="NPC_MagoOscuro",
        mano=VFX_DESTELLO_MANO, proyectil=VFX_BOLA_DE_FUEGO, impacto=VFX_EXPLOSION,
        sfxLanza="Star_SpellCast", sfxImpacto="Impact1", velocidad=14.0, carga=0.25,
        sacudida=0.3, esperar=True))

    # ── 2b. La del aire (fase 8), que tenia el mismo fallo por otro camino ──────────────────
    #
    # Va en un Parallel que no espera a todos, para cortar al Mago Oscuro con la bola ya en el
    # aire. Bien. Pero el TakeDamage iba 0,3 s despues del lanzamiento y el vuelo dura casi un
    # segundo. Ahora se espera al impacto justo antes de la reaccion.
    i_aire = _plano_con(final, "Le da en el aire")
    assert F[i_aire + 1].tipo == "GestureBeat" and F[i_aire + 1].get("gesture") == s("TakeDamage")
    F.insert(i_aire + 1, __import__("dsl")._beat("EsperarHechizoBeat", "Hasta que le llega. Sin esto se dolia antes "
                                "de que la bola le alcanzara.",
                                [("hechizo", s("HECHIZO_ARCHIMAGO")),
                                 ("topeDeSalida", f(1.5)), ("topeDeVuelo", f(6.0))]))
    # «Y CAE»: un segundo plano vivo del mismo personaje justo detras del primero. Era un corte
    # entre dos encuadres casi iguales — el salto que Raul describe. El vivo de antes ya le sigue.
    F.pop(_plano_con(final, "Y CAE."))

    # ── 3. El plano de la foto (4:00): fuera, y el picado entra antes ───────────────────────
    #
    # «Los dos: el arriba, el otro abajo» era un general a ras de suelo, y el Mago Oscuro salta
    # siete metros DESPUES de que se ponga: se salia por arriba y quedaba cesped. El plano vivo del
    # picado ya le sigue pegado; se adelanta a ese sitio y acompana el despegue, la subida y la
    # bajada sin un solo corte.
    F.pop(_plano_con(final, "Los dos: el arriba, el otro abajo"))
    picado = F.pop(_plano_con(final, "EL PICADO"))
    picado.set("note", s("VIVO desde que toma impulso: la camara le sigue en el despegue, la "
                         "subida y el picado, sin cortes. Sustituye al general a ras de suelo de "
                         "la foto del 21 sep, en el que se salia por arriba."))
    i_imp = _uno(final, lambda b: b.tipo == "GestureBeat" and "Toma impulso" in _nota(b), "Toma impulso")
    F.insert(i_imp, picado)

    # ── 4. El escudo se rompe y sale despedido ──────────────────────────────────────────────
    i_bajada = _uno(final, lambda b: b.tipo == "ParallelBeat" and "se cubre mientras baja" in _nota(b), "picado")
    for h in F[i_bajada].hijos:
        if h.tipo == "VfxBeat":
            # El escudo se apaga en el choque, no un segundo despues: se ha roto.
            h.set("lifetime", f(1.2))
    F.insert(i_bajada, mirar("NPC_Archimago", hacia="NPC_MagoOscuro", giro=0.25,
                             note="De cara a lo que baja: al romperse el escudo sale despedido "
                                  "HACIA ATRAS, y atras depende de adonde mire."))

    i_choque = _uno(final, lambda b: b.tipo == "VfxBeat" and "El choque, sobre el escudo" in _nota(b), "choque")
    F.insert(i_choque + 1, vfx(VFX_ESTRELLA, "El escudo, rompiendose.", actor="NPC_Archimago",
                               offset=(0, 1.0, 0), vida=1.2))

    # Lo que habia: DefendHit, «Su cara, aguantando» y un segundo de espera en guardia — la
    # segunda foto. Fuera los tres.
    F.pop(_uno(final, lambda b: b.tipo == "GestureBeat" and b.get("gesture") == s("DefendHit_NoWeapon"), "DefendHit"))
    i_aguanta = _plano_con(final, "Su cara, aguantando")
    assert F[i_aguanta + 1].tipo == "WaitBeat"
    del F[i_aguanta:i_aguanta + 2]

    i_aterriza = _uno(final, lambda b: b.tipo == "GestureBeat" and "Toma tierra al final del picado" in _nota(b), "Landing")
    aterriza = F.pop(i_aterriza)

    lanzado = saltar("NPC_Archimago", "Le rompe el escudo y le LANZA: hacia arriba y hacia atras, "
                     "en parabola, cayendo. Acaba tirado en el suelo.",
                     altura=2.2, desplazamiento=-2.8, subida=0.42, caida=0.55,
                     poseSubida="Falling", poseAire="Falling", poseCaida="")
    lanzado.set("parabola", "true")
    lanzado.set("poseEnElSuelo", s("Die01Stay_NoWeapon"))

    derribo = [
        plano("Le sigue mientras vuela. VIVO, general y un poco alto: se tiene que ver entera la "
              "parabola y donde cae.", "Wide", "NPC_Archimago", altura=1.2, distancia=1.3,
              vivo=True, encara=False),
        a_la_vez("Sale despedido; el otro toma tierra.", [lanzado, aterriza]),
        esperar(0.6, "En el suelo, un momento."),
        mantener("NPC_Archimago", "GetUp_NoWeapon", "Y se levanta. GetUp arranca justo de la pose "
                 "de tirado en el suelo, y sin bucle se queda de pie en su ultimo fotograma."),
        esperar(1.0, "Lo que dura levantarse (25 fotogramas)."),
        soltar("NPC_Archimago", "De pie otra vez."),
    ]
    i_despega = _uno(final, lambda b: b.tipo == "GestureBeat" and "Y vuelve a despegar" in _nota(b), "despega")
    F[i_despega:i_despega] = derribo
    return fases


def pase_prologo12(fases):
    """
    Grabacion 12 (21 sep):

    1. «Ese escudo se baje»: el anillo del escudo le quedaba a la cintura. El pivot del prefab
       esta en el centro de la esfera; todos los escudos del Archimago bajan a -0,1.
    2. «Cuando el Mago Oscuro camina hacia el Archimago antes del duelo va muy lento.»
    3. «El Mago Oscuro le alcanza con un hechizo y se queda como en la imagen 2 (de pie, quieto);
       nada mas llegar deberia hacer la animacion de la imagen 3 (Pain01).» El suelo que se abre:
       TakeDamage_2 de medio segundo, deslizarse dos metros andando y un segundo parado ANTES del
       dolor. Ahora el dolor es el golpe: sale despedido dos metros con Pain01 puesto desde el
       primer fotograma y se queda asi un momento.
    4. «Tenemos animaciones para simular perfectamente el momento de la imagen 4» (tirado boca
       abajo con las piernas arriba): la pose fija Die01Stay aparecia de golpe al aterrizar. Ahora
       vuela con Die01, el derribo del pack, que va de pie a tirado, y se queda en su ultimo
       fotograma hasta que se levanta.
    5. «Absoluta se pierde entre las casas, deberia saltar mas arriba.»
    """
    duelo = por_nombre(fases, "7 - El duelo")
    final = por_nombre(fases, "8 - El ultimo hechizo")

    # 1
    def bajar_escudos(beats):
        n = 0
        for b in beats:
            if b.tipo == "VfxBeat" and VFX_ESCUDO in (b.get("vfxPrefab") or ""):
                b.set("offset", v3(0, -0.1, 0))
                n += 1
            if b.hijos:
                n += bajar_escudos(b.hijos)
        return n
    n = sum(bajar_escudos(fs.beats) for fs in fases)
    assert n >= 3, f"esperaba al menos 3 escudos, hay {n}"

    # 2
    D = duelo.beats
    entra = _uno(duelo, lambda b: b.tipo == "WalkPathBeat" and b.get("actorId") == s("NPC_MagoOscuro")
                 and "Los ultimos metros" in _nota(b), "entrada del Mago Oscuro")
    D[entra].set("speed", f(2.4))

    # 3
    i_dano = _uno(duelo, lambda b: b.tipo == "GestureBeat" and b.get("gesture") == s("TakeDamage_2")
                  and b.get("actorId") == s("NPC_Archimago"), "TakeDamage_2")
    assert D[i_dano + 1].tipo == "MoveToBeat" and "despedido" in _nota(D[i_dano + 1])
    assert D[i_dano + 2].tipo == "WaitBeat"
    polvo = D[i_dano + 3]
    assert polvo.tipo == "VfxBeat"
    dolor = D[i_dano + 4]
    assert dolor.tipo == "GestureBeat" and dolor.get("gesture") == s("Pain01")
    golpe = saltar("NPC_Archimago", "El suelo se abre y le LANZA dos metros hacia atras, con el "
                   "dolor puesto desde el primer fotograma (Pain01): nada de quedarse de pie.",
                   altura=0.6, desplazamiento=-2.0, subida=0.18, caida=0.3,
                   poseSubida="Pain01", poseAire="Pain01", poseCaida="")
    golpe.set("parabola", "true")
    golpe.set("poseEnElSuelo", s("Pain01"))
    D[i_dano:i_dano + 5] = [
        polvo,
        golpe,
        esperar(0.8, "Doblado, un momento. Sin esto el golpe no se siente."),
        soltar("NPC_Archimago", "Se recompone."),
    ]

    # 4
    F = final.beats
    i_lanza = _uno(final, lambda b: b.tipo == "ParallelBeat" and "Sale despedido" in _nota(b), "lanzado")
    salto = next(h for h in F[i_lanza].hijos if h.tipo == "SaltoBeat")
    salto.set("poseSubida", s("Die01_NoWeapon"))
    salto.set("poseAire", s("Die01_NoWeapon"))
    salto.set("poseEnElSuelo", s("Die01_NoWeapon"))
    salto.set("note", s("Le rompe el escudo y le LANZA en parabola, cayendo con Die01 (el derribo "
                        "del pack: de pie a tirado). Se queda en su ultimo fotograma hasta GetUp."))

    # 5
    i_abs = _uno(final, lambda b: b.tipo == "SaltoBeat" and b.get("actorId") == s("NPC_Archimago")
                 and "EL SALTO" in _nota(b), "salto de Absoluta")
