// Copia para la maqueta: conserva el agua de MainWorld y corrige su profundidad en cámara ortográfica.
Shader "El Sendero/Eldoria/Agua MainWorld corregida"
{
    Properties
    {
        [Header(Surface)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Interpolators
            {
                ...
                float profundidadOjo : TEXCOORD3;
            ...
            void VertexFunction(Attributes attribs, out Interpolators varyings)
            {
	            ...
	            varyings.profundidadOjo = -TransformWorldToView(position_ws.xyz).z;
            ...
            float ProfundidadOjo(float muestra)
            {
                if(unity_OrthoParams.w > .5)
                {
                    ...
                    muestra = 1 - muestra;
                    #endif
                    return lerp(_ProjectionParams.y,_ProjectionParams.z,muestra);
                }
                return LinearEyeDepth(muestra,_ZBufferParams);
            }
            void FragmentFunction(Interpolators varyings, out half4 outColor : SV_Target)
            {
	            ...
	            half depth = max(0,ProfundidadOjo(SampleSceneDepth(uv_ss)) - varyings.profundidadOjo);
	            half depthMask = saturate(depth - _Depth);

	            // Calculating Refracted Depth
	            half refDepth = max(0,ProfundidadOjo(SampleSceneDepth(uv_ss + normal.xz * _Refraction)) - varyings.profundidadOjo);
