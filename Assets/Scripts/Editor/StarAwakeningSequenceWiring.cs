...
public static class StarAwakeningSequenceWiring
{
    ...
    public static void Wire()
    {
        ...
        // El icono del panic input es un caso distinto: RescueIfStranded clona un script SIN
        // vista, que sirve para lógica pura (arriba), pero panicInputUI es un Canvas con hijos
        // (fondo, icono, barra de progreso) — clonar solo el script dejaría esos hijos donde
        // estaban, es decir, sin dibujarse igual. Aquí se reparenta el Canvas entero.
        RescuePanicInputUIIfStranded(soModule, "panicInputUI", go.transform, log, warnings);
    ...
    /// Variante para el icono del panic input (INC-224, 17 sep 2026): el vídeo de Raúl mostraba
    /// el botón de la X saliendo (INC-221 ya arreglado, el detector cuenta bien sus 5 s reales)
    /// pero el icono nunca aparecía en pantalla, sin ningún error — 'Activate()' se ejecutaba
    /// entero, sin excepciones. Causa: 'panicInputUI' vivía en el mismo GameObject viejo que el
    /// resto ('dentro del rig DIRECTOR del sequencer', ver incidencia del 5 sep sobre el bocadillo
    /// de Eldran), así que quedó igual de apagado que el detector — pero es un Canvas con hijos
    /// (fondo, icono, barra), no un script suelto: clonar solo el componente 'PanicInputUI' con
    /// 'RescueIfStranded' habría dejado sus referencias (_rootGroup, _buttonIcon, _progressBar)
    /// apuntando a los hijos de siempre, todavía colgados del objeto apagado. Por eso esto no
    /// clona nada: reparenta el Canvas ANCESTRO más cercano (que se basta a sí mismo — trae ya
    /// sus propios hijos) a un sitio activo.
    private static void RescuePanicInputUIIfStranded(SerializedObject soModule, string field,
        Transform newParent, StringBuilder log, List<string> warnings)
    {
        var prop = soModule.FindProperty(field);
        var ui = prop?.objectReferenceValue as PanicInputUI;
        if (ui == null) return;
        if (ui.gameObject.activeInHierarchy) return; // ya funciona: no tocar nada

        string strandedName = ui.gameObject.name;
        var canvas = ui.GetComponentInParent<Canvas>(true);
        var toMove = canvas != null ? canvas.transform : ui.transform;

        bool alreadyReparented = (toMove.parent == newParent);
        if (!alreadyReparented)
        {
            Undo.SetTransformParent(toMove, newParent, "Rescatar el icono del panic input");
        }

        // Reparentar solo arregla la jerarquía. Si el propio Canvas (no un antecesor) tiene
        // su flag de activo a false, seguirá sin verse aunque cuelgue del sitio correcto —
        // por eso hay que forzar también su propia activación, por separado.
        bool selfWasInactive = !toMove.gameObject.activeSelf;
        if (selfWasInactive)
        {
            toMove.gameObject.SetActive(true);
        }

        EditorUtility.SetDirty(toMove.gameObject);

        if (!toMove.gameObject.activeInHierarchy)
        {
            // Ni reparentar ni activarlo a él mismo ha bastado: algún antecesor por encima de
            // newParent también está apagado. Eso ya no lo puede arreglar este montaje a ciegas.
            warnings.Add($"El icono del panic input (PanicInputUI, en '{strandedName}') sigue sin " +
                         $"poder mostrarse aunque su Canvas ('{toMove.name}') ya está bajo " +
                         $"'{newParent.name}' y activo por sí mismo: algún antecesor por encima de " +
                         $"'{newParent.name}' debe seguir apagado. Revísalo a mano en el Inspector.");
            return;
        }

        if (alreadyReparented)
        {
            log.AppendLine($"PanicInputUI RESCATADO (arreglo definitivo): su Canvas " +
                           $"('{toMove.name}') ya colgaba de '{newParent.name}' desde el montaje " +
                           "anterior, pero seguía DESACTIVADO EN JERARQUÍA porque él mismo (no un " +
                           "antecesor) tenía su propio flag de activo a false — reparentar no toca " +
                           "eso, solo mueve la jerarquía. Se ha forzado su activación. Con esto el " +
                           "icono de la X ya debería dibujarse.");
        }
        else
        {
            log.AppendLine($"PanicInputUI RESCATADO: su Canvas ('{toMove.name}') estaba DESACTIVADO " +
                           "EN JERARQUÍA (colgando, con 'panicInputUI', del GameObject viejo del " +
                           "sequencer). Por eso 'Activate()' se ejecutaba entero sin ningún error pero " +
                           "el icono nunca llegaba a dibujarse. Reparentado a " +
                           $"'{newParent.name}'" + (selfWasInactive
                               ? " y activado él mismo (también tenía su propio flag a false)"
                               : "") + ". A diferencia de los otros rescates, aquí no se clona " +
                           "nada — se mueve el Canvas completo para no dejar huérfanos a sus hijos " +
