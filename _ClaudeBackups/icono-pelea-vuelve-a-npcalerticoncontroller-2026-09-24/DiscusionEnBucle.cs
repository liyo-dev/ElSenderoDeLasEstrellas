...
namespace Game.NPC
{
    ...
    public class DiscusionEnBucle : MonoBehaviour
    {
        ...
        [Tooltip("Altura mínima (m, desde los pies) del punto sobre el que flota el bocadillo. Si la " +
                 "cabeza queda más arriba, se usa la cabeza + 0,2 m. Misma regla que NPCAlertIconController.")]
        [SerializeField] private float alturaDelIcono = 2.2f;
        [Tooltip("Lo que sube el bocadillo por encima de ese punto (m).")]
        [SerializeField] private float margenSobreLaCabeza = 0.3f;
        ...
        {
            _discutiendo = false;
            // Barrer ANTES de quitar los dos propios (INC-415). Al revés, Quitar() los deja a
            // null con el Destroy aún pendiente hasta fin de frame, el barrido los encontraba y
            // los contaba como «sueltos»: el aviso de INC-388 decía que había bocadillos de más
            // cuando no los había, y mandó la investigación de los iconos dobles por mal camino.
        ...
        private GameObject Icono(Transform npc)
        {
            ...
            // INC-428: «les tapa la cara». Antes iba a la altura del hueso de la cabeza + 0,35 m,
            // y el hueso de la cabeza está en la base del cráneo: con un icono de un metro de
            // ancho, el círculo caía justo delante de la cara. Ahora, la misma regla que usaban
            // los iconos de NPCAlertIconController: a 2,2 m de los pies como mínimo (o 0,2 m por
            // encima de la cabeza si el NPC es más alto) y 0,3 m más.
            float cabeza = AlturaDeLaCabeza(npc);
            float altura = Mathf.Max(alturaDelIcono, cabeza + 0.2f) + margenSobreLaCabeza;
            Vector3 donde = npc.position + Vector3.up * altura;

            // Y el efecto de aparición, el bote y el mirar a cámara que tenían aquellos iconos.
            if (!go.TryGetComponent(out IconoSobreCabeza icono)) icono = go.AddComponent<IconoSobreCabeza>();
            icono.Aparecer(donde, iconoDePelea.transform.localScale);
            return go;
        }

        /// Se encoge antes de desaparecer (INC-428); si no lleva el efecto, se borra sin más.
        private static void Quitar(ref GameObject icono)
        {
            if (icono != null)
            {
                if (icono.TryGetComponent(out IconoSobreCabeza efecto)) efecto.Desaparecer();
                else Destroy(icono);
            }
            icono = null;
        }
