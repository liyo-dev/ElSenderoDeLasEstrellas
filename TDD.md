...
**Última auditoría general documentada:** 12 de agosto de 2026 — ver § 19.4. El estado actual de las incidencias y su validación se consulta en `TRACKER.md`.
...
| GamepadInputReader | `Core/GamepadInputReader.cs` | Input centralizado con eventos |
...
- **Trampa conocida (ver INC-086, TRACKER.md / § 13):** `NPCTeamMember` no viene en el prefab — lo añade dinámicamente `NPCCombatTeam.Start()` (`AddComponent` + `SetTeam`). Cualquier componente que necesite `GetComponent<NPCTeamMember>()` debe resolverlo de forma perezosa (primer uso real), **nunca cachearlo en su propio `Awake()`**: en ese momento el componente todavía no existe, porque Unity ejecuta todos los `Awake()` de la escena antes que cualquier `Start()`.
...
**Actualización — 12 de septiembre de 2026:** para el detalle de qué componente legacy concreto sustituye cada nodo del grafo (tabla de equivalencias, catálogo completo de los 37 tipos de nodo con los 7 marcados `Obsolete`, y la lista honesta de huecos que aún faltan por construir), ver `claude/catalogo-sistemas-legacy-vs-grafo-nuevo-2026-09-12.md` en el Project. Este documento es la referencia viva del día a día; se actualiza a medida que se construyen nodos nuevos, mientras que esta sección solo recoge la política general.
...
### Presentación cinemática reversible de MainMenu

`Assets/Scripts/Editor/MainMenuCinematicDesignBuilder.cs` ofrece dos comandos en `El Sendero → MainMenu`:

- **Crear vista previa cinemática (segura):** copia `MainMenu.unity` a `MainMenu_Cinematica_Preview.unity`, genera las capas visuales y abre la copia. No añade la vista previa a Build Settings ni guarda cambios en la escena original.
- **Aplicar diseño cinemático al menú funcional:** hace primero una copia fechada en `Assets/Scenes/Systems/Backups/` y después aplica el mismo diseño a `MainMenu.unity`.
...
**Nota (24 ago 2026) — numeración `INC-XXX` unificada en `TRACKER.md`.** Los bugs de la "Sesión Agosto 2026" más abajo llevaban su propia numeración `INC-075`–`INC-084`, que coincidía por casualidad de formato con IDs ya en uso en el Tracker de Incidencias (antes en Google Sheets, hoy `TRACKER.md`) para incidencias completamente distintas. Se renumeraron a `INC-083`–`INC-092` para que cada ID sea único en todo el proyecto — ver la tabla de correspondencia en `TRACKER.md`. Esta sección conserva el detalle técnico completo (causa raíz, código, diagnóstico); `TRACKER.md` lleva la fila corta de seguimiento y el estado de validación en juego.
...
- **INC-083 (`EstelaAppearsSequencer.cs`)** — la clave de flag "ya vista" que se añadió el 27-28/jul ("CINEMATIC_SEEN:ESTELA_APPEARS") no seguía la convención real del proyecto ("CINEMATIC_SEEN:Cinematic_{id}", la que ya usa `SimpleCinematicDirector` y la que llevan escrita los 9 `PlayerPreset_*.asset` existentes como "CINEMATIC_SEEN:Cinematic_EstelaAppears"). Al activar modo test contra cualquiera de esos presets (p.ej. `PlayerPreset_Taberna`, activado el 05/08), la clave nunca coincidía y `HasSequencePlayed()` daba `false` pese a que el preset sí registraba la cinemática como vista → arañas y guerreros reaparecían en `Awake()`. Se añadieron `CinematicSequencerBase.HasCinematicBeenSeen(id)`/`MarkCinematicAsSeen(id)` con la convención correcta y `EstelaAppearsSequencer` los usa ahora; se mantiene lectura de compatibilidad con la clave antigua por si algún save real (no preset) quedó escrito con ella entre el 27/jul y el 05/ago. Revisados también `TabernaSequencer`/`LiamGolemSummonSequencer` (únicos otros con flag `Cinematic_*` ya presente en los presets): ninguno de los dos desactiva actores de forma permanente, así que no necesitan el mismo mecanismo de ocultación — sus flags son solo marcadores narrativos.
- **INC-084 — jugador "pillado" tras fallar el Despertar de la Estrella (`AWAKEN_FAILED`).** En `MainNarrative_Cap1.asset`, el nodo `RaiseCustomEventNode` que emite `AWAKEN_START` (guid `0c0af6c1...`) hace fork en dos `WaitCustomEventNode`: uno espera `AWAKEN_DONE` (éxito), otro `AWAKEN_FAILED` (fallo). La rama de fallo vuelve a entrar en `WaitQuestCompleteNode`/`GiveInventoryItemNode` (no-ops porque la quest ya está completa y el ítem ya se dio) y de ahí de vuelta al mismo nodo fork, para reintentar la cinemática (`StarAwakeningSequencer`) — patrón ya reconocido en el código de `NarrativeRunner.RunSubGraph` (fork "revisitado en vivo", añadido para evitar el `StackOverflowException` que este mismo reintento causaba antes). Causa raíz de que el reintento se quedara "pillado" en vez de resetearse limpio: (1) `WaitCustomEventNode` marcaba su flag `__event_{guid}_{key}_received` en el blackboard como permanentemente `true` tras recibir el evento una vez — pensado solo para el resume tras recarga (ver `SavePoint`), pero al no limpiarse nunca, la segunda vuelta del bucle de reintento leía ese flag viejo y avanzaba sin esperar el evento real, disparando un reintento fantasma inmediato de `AWAKEN_START`. (2) Dos `StarAwakeningSequencer.Co_Sequence()` solapados llamaban ambos a `LockCinematic()` (dos `PushMode(ActionMode.Cinematic)`), pero `EndCinematic()` solo restaura una vez por el guard de `_cinematicLocked` → un `Push` se quedaba sin su `Pop` y el jugador perdía el control para siempre. **Arreglado** (Agosto 2026): `WaitCustomEventNode.Enter()` consume su propio flag al leerlo (lo deja en `false` en vez de dejarlo en `true` para siempre); `CinematicSequencerBase` ahora ignora la señal de entrada si ya hay una secuencia en curso (`_sequenceRunning`), garantizando `Push`/`Pop` siempre equilibrados pase lo que pase en el grafo; `NarrativeRunner` añade un contador de "generación" por nodo-fork (`_forkGeneration`, en memoria) para que las ramas de un fork que quedan obsoletas tras un re-fork en vivo se descarten en vez de duplicar el tramo de éxito (`UnlockAbilitiesNode` → `StartQuestNode` → `StartBattleNode`) si el evento de éxito llega igualmente para una rama abandonada. No se tocó `WaitQuestCompleteNode` ni la lógica de resume (`node == start`) de `StartFromStartNode`/`RelaunchForkBranches`.
- **INC-085 (16 ago 2026) — `AbilityUnlockPopupUI.cs` / `SceneBoundUI.cs` — el popup de desbloqueo de habilidad/hechizo dejó de verse.** Reportado específicamente para el desbloqueo del primer hechizo ("Bola Prisma") al final del Despertar de la Estrella. Causa raíz: `StarAwakeningSequencer.RaiseSignal(AWAKEN_DONE)` dispara síncronamente (ver nota en el propio archivo, línea ~341) `UnlockAbilitiesNode → StartQuestNode → StartBattleNode → BossIntroPresentation.PlayIntroduction()`, y esta última llama a `SceneBoundUI.BeginBossIntro()` **antes de su primer `yield`** — es decir, en el mismo frame en que `UnlockAbilitiesNode` acaba de disparar `UnlockService.OnSpellUnlocked` y `AbilityUnlockPopupUI.AnimateIn()` ya ha arrancado su animación de entrada. `BeginBossIntro()` añade (o reutiliza) un `CanvasGroup` en la raíz del propio GameObject `AbilityUnlockPopupUI` (distinto del `popupCanvasGroup` hijo que anima `AnimateIn()`) y lo funde a alpha 0 para ocultar toda la UI persistente — sin consultar en ningún momento el guard `ISceneBoundUIHideGuard`/`BlocksSceneHide()` que sí respeta `ApplySceneState()` para cambios de escena. Como el alpha de un `CanvasGroup` hijo se multiplica por el del padre, el popup quedaba invisible aunque su propia animación de entrada se ejecutara con normalidad. La comprobación existente `if (SceneBoundUI.IsBossIntroActive)` al principio de `ShowPopup()` no cubre este caso porque, en el momento en que se evalúa, la intro del boss todavía no ha empezado — empieza unas líneas de código (síncronas) después, dentro de la misma llamada. **Arreglado:** `BeginBossIntro()` ahora respeta `_hideGuard.BlocksSceneHide()` igual que `ApplySceneState()` — si el popup sigue en pantalla, no le toca el `CanvasGroup` y marca el fundido como pendiente (`_pendingBossIntroHide`); `ReapplySceneState()` (ya invocado por `AbilityUnlockPopupUI.HidePopup()` al terminar) aplica ese fundido pendiente si la intro del boss sigue activa, o no hace nada si ya terminó. `EndBossIntro()` limpia el flag pendiente sin tocar el `CanvasGroup` de esas instancias (nunca se llegó a fundir). No se tocó `AbilityUnlockPopupUI.cs` ni ningún prefab/escena — el guard ya estaba correctamente cableado (`AbilityUnlockPopupUI` implementa `ISceneBoundUIHideGuard` en el mismo GameObject), solo faltaba que `BeginBossIntro`/`EndBossIntro`/`ReapplySceneState` lo consultaran.
- **INC-086 (16 ago 2026) — `NPCCombatLifecycleHandler.cs` — en combate de dúo/grupo, la música y animación de victoria sonaban al derrotar al PRIMER enemigo (no al último), y la música de combate se restauraba (se quitaba) justo después de esa primera derrota en vez de esperar a que cayera todo el equipo.** Reportado por Raúl justo después del rediseño de combate en dúo del 15 ago 2026 (cada NPC dice su propia frase de derrota, ver bullet de arriba y sección 3 — Equipos de combate). Causa raíz: ese mismo rediseño incluyó el "FIX #25" — cachear `_teamMember = GetComponent<NPCTeamMember>()` una sola vez en `Awake()` para evitar 5 `GetComponent` sueltos (`OnDied`/`DeathRoutine`/`HandleGetUpDizzy`/`HandleMoveToAnchor`/`MoveTeamMembersToRandomPoints`). Pero `NPCTeamMember` no viene en el prefab: lo añade dinámicamente `NPCCombatTeam.Start()` (`AddComponent<NPCTeamMember>()` + `SetTeam(...)`), y Unity ejecuta **todos** los `Awake()` de la escena antes que **cualquier** `Start()`. En el instante de ese `Awake()` el componente todavía no existía en el GameObject, así que `GetComponent<NPCTeamMember>()` devolvía `null` — y como solo se asignaba una vez, quedaba `null` para siempre (nunca se refrescaba). Con `_teamMember` siempre `null`, `isInTeam` era siempre `false` para cualquier NPC de equipo, así que cada muerte se trataba como "combate terminado en solitario": `shouldCelebrate` en `DeathRoutine()` y la restauración de música en `HandleGetUpDizzy()` disparaban en el primer enemigo derrotado, no en el último de todos. **Arreglado:** se sustituyó el campo cacheado en `Awake()` por una propiedad `ResolvedTeamMember` que resuelve `GetComponent<NPCTeamMember>()` de forma perezosa — en el primer uso real, ya en combate y con el componente ya añadido por `NPCCombatTeam.Start()` — y lo cachea a partir de ahí. Los 5 puntos de uso ahora leen `ResolvedTeamMember` en vez del campo crudo `_teamMember`. No se tocó la lógica de `isLastTeamMember`/`shouldCelebrate` en sí (ya estaba bien escrita) — solo el dato de entrada (`teamMember`) que recibía, que llegaba mal por la carrera Awake/Start. **Trampa a no repetir:** ningún componente debe cachear `GetComponent<NPCTeamMember>()` (ni de ningún otro componente que `NPCCombatTeam` añada dinámicamente) en su propio `Awake()` — solo en `Start()` en adelante, o de forma perezosa.

- **INC-087 (19 ago 2026) — cierre definitivo de "Will invisible" (secuela final del bug documentado en § 4, "Sistema del Jugador"; `ActiveCharacterSwapper.cs`).** Reportado por Raúl con logs reales: `[ActiveCharacterSwapper] 🩺 Will NPC en encuadre pero SIN renderizar durante ~1s ... Aplicando autocuración` disparándose repetidamente al cambiar de personaje en ciertos puntos del mapa, pese a las tres rondas de fix previas (`updateWhenOffscreen`, lectura forzada de `.bounds`, `Animator.Update(0f)`, `allowOcclusionWhenDynamic=false` contra el Occlusion Culling horneado de `MainWorld.unity`, y el heal periódico `DiagnoseAndHealWillVisibility()` que toggla `Renderer.enabled`). Diagnóstico a partir del propio latido de diagnóstico 🫀 que ya emitía el heal: `rendererBounds` quedaba clavado en `center=(1561.96, 1001.40, 653.23)` mientras la posición real de Will era `(1414.76, 1001.01, 9.94)` — ~650 unidades de diferencia en Z, sin moverse ni un ápice entre latidos de 2,5s espaciados varios segundos. Eso descarta culling legítimo: es el AABB del `SkinnedMeshRenderer` completamente congelado, nunca recalculado desde el spawn. **Causa raíz real** (nunca tocada por las tres rondas anteriores, todas ellas parches reactivos sobre el síntoma): el rig de Will/Liam/Estela corre su `Animator` en `AnimatorCullingMode.CullUpdateTransforms` (valor por defecto del componente). Con ese modo, mientras el renderer no se considera "visible" el Animator deja de actualizar los huesos — y sin huesos moviéndose, el `SkinnedMeshRenderer` nunca recalcula sus bounds, así que nunca vuelve a considerarse visible: el mismo punto muerto que ya diagnosticaba el comentario original de `SpawnWillNpc()`, pero nunca resuelto de raíz. `Animator.Update(0f)` solo empujaba un frame puntual a través del interbloqueo; si ese frame no bastaba para que Unity marcara el renderer visible antes del siguiente test de culling, el interbloqueo se rearmaba de inmediato — de ahí que `updateWhenOffscreen` + `.bounds` forzado + el toggle de `Renderer.enabled` del heal periódico no lo cerraran del todo. **Arreglado:** `cullingMode = AnimatorCullingMode.AlwaysAnimate` en los tres puntos del Animator de Will/Liam/Estela — `SpawnWillNpc()` (`ActiveCharacterSwapper.cs:699`), `WarpNpcToPosition()` (`:1031`, cubre también a Liam/Estela residentes en escena que nunca pasan por `SpawnWillNpc()`) y el heal periódico `DiagnoseAndHealWillVisibility()` (`:383`, red de seguridad por si algo revierte el valor). Saca al Animator del modo que causaba el interbloqueo en vez de seguir empujando frames sueltos contra él: huesos y bounds se actualizan todos los frames, on-screen o no, así que no queda ningún estado "nunca visible" del que depender para salir del bucle. Coste real: 3-4 NPCs de equipo, no cientos — irrelevante en el profiler. **Confirmado por Raúl en juego (19 ago 2026): incidencia dada por solucionada.**

- **INC-088 (20 ago 2026) — bug "dos Estelas" al cargar partida — `PlayerPresetService.cs` (`IsSelectionCorrupted`).** Reportado por Raúl con captura de pantalla (dos modelos idénticos de Estela en el grupo que sigue a Will) y log completo de una carga real. El propio proyecto ya tenía nombrado y parcialmente mitigado este bug como "dos Estelas y un Will" en `ActiveCharacterSwapper.cs` (`EnsureHiddenNpcSuppressed`, red de seguridad cada 0.5s), pero esa mitigación solo cubre la mitad del problema: el NPC "oculto" (Liam/Estela) recuperando renderers por accidente mientras el controller lo representa. Este incidente es la otra mitad, nunca cubierta: el controller (el rig de Will, reutilizado para representar a quien esté activo) queda vestido con la apariencia de Estela **aunque el personaje activo sea Will**, mientras la Estela real del equipo sigue siendo un NPC aparte, visible y siguiendo a Will con normalidad — de ahí las dos Estelas simultáneas (más Will si además hay un `_willNpcInstance` de equipo en pantalla). **Causa raíz, con evidencia del log real:** `PlayerPresetService.InitializePresetService()` toma un "pre-snapshot" de Will (`CharacterAppearanceRegistry.SnapshotWillFromBuilderIfNeeded()`) leyendo lo que el builder muestra en ese instante del boot, ANTES de aplicar nada — pensado como referencia para detectar `preset.appearance` corrompido (`IsSelectionCorrupted()`). En la partida reportada ese pre-snapshot salió `[Body:Body09, Head:Head02_Female, Hair:Hair08, Eyes:Eye02, Mouth:Mouth02, Accessory:AC09_Ribbon]` — la apariencia de **Estela**, no la de Will (la causa de que el builder arrancara ya mostrando eso no se ha perseguido más allá — no hace falta para el fix: el bug real es que `IsSelectionCorrupted()` confiaba ciegamente en esa referencia sin validarla). Con la referencia (`willSnap`) ya corrompida, la selección REAL y correcta del preset (`[Body:Body05, Cloak:Cloak02, Head:Head01_Male, Hair:Hair01, Eyes:Eye01, Mouth:Mouth01]`, la apariencia auténtica de Will) "difiere del snapshot" en todas sus categorías — y como `Eyes:Eye01`/`Mouth:Mouth01` resultan ser partes genéricas que también figuran en la apariencia registrada de Liam, `IsSelectionCorrupted()` contó 2 coincidencias y marcó el preset (el único dato correcto de los dos) como "corrupto": log real `[PlayerPresetService] 🔍 2 partes de otro personaje detectadas en preset.appearance.` seguido de `⚠️ Corrupción detectada en preset.appearance. Aplicando snapshot existente del registry sin sobreescribir.` — es decir, se descartó la apariencia correcta de Will y se re-aplicó al builder la apariencia de Estela, dejándolo permanentemente vestido de Estela. **Arreglado:** `IsSelectionCorrupted()` ahora empieza comprobando que su propia referencia (`willSnap`) no se parezca ya a Estela/Liam (`LooksLikeOtherCharacter()`, nuevo helper) — si el pre-snapshot ya "es" otro personaje, no es un punto de comparación fiable y se omite la detección por completo, confiando en `preset.appearance` (que `SnapshotAppearanceToPreset()` solo escribe cuando el personaje activo es Will, así que por diseño debería llegar limpio). No se ha tocado la detección de corrupción en sí para el caso en que `willSnap` sí sea válido — sigue protegiendo contra el incidente original para el que se añadió. **Pendiente de confirmar por Raúl en juego** (no se ha podido reproducir en el Editor desde esta sesión) — si el pre-snapshot corrupto vuelve a aparecer con este fix puesto, el siguiente paso sería perseguir por qué el builder arranca mostrando la apariencia de Estela antes de que `PlayerPresetService` llegue a aplicar nada.

- **INC-089 (20 ago 2026) — "la cámara está en otro sitio" en diálogos grupales tras un teletransporte — `DialogueCinematicController.cs` (`StartCinematic`).** Reportado por Raúl con captura de pantalla (durante el diálogo grupal de los "arrestados", jugando con Estela activa: la cámara quedaba encajada contra un muro, viendo el grupo real a través de un hueco/puerta, en vez de encuadrar la sala donde de verdad transcurre la escena) y la descripción del patrón: "cuando controlamos a un personaje que no es Will, y vamos a una conversación grupal que transcurre en otro espacio — es decir, hay un teletransporte previo al diálogo grupal — la cámara se rompe". El archivo ya tenía un historial extenso (agosto 2026) de fixes para "cámara de grupo cuando el personaje activo no es Will" (filtrar `ActiveCharacterSwapper.HiddenNpc` del centro del grupo en `CalculateCameraPosition()`, `UpdateGroupFacing()`, el *breathing* de `LateUpdate()` y `FindSpeakerTransform()`) — todos esos filtros seguían correctos y no eran la causa aquí. **Causa raíz real, sin relación con el decoy:** el patrón narrativo típico es `NPCInteractiveNarrativeExecutor` encadenando `[Dialogue]` (p.ej. el guardia habla antes de encerrar al grupo) → `TeleportPlayer` (`ExecuteTeleportPlayer`, mueve solo a `PlayerService.Player`, el rig único — no hace `Physics.SyncTransforms` ni falta que le haga) → `[Dialogue]` grupal ya en la celda. `ExecuteDialogue()` pasa siempre `transform` (el propio GameObject del executor) como "NPC" a `DialogueManager.StartDialogue`, así que **el mismo transform de NPC dispara ambos diálogos encadenados**. `StartCinematic()` detecta diálogos encadenados dentro de un grace period de 0.2s (`chainDialogueGracePeriod`, para no parpadear cámara entre líneas consecutivas) comprobando `currentNPC == npc && currentPlayer == player` — con el mismo executor y el mismo rig, esa condición era CIEMPRE cierta entre el diálogo previo y el grupal, así que la rama de "reutilizar cinematográfica activa" se colaba siempre que el `TeleportPlayer` intermedio tardaba menos de 0.2s reales en ejecutarse (instantáneo si el `entry.teleportTransition` no cubre pantalla, o el resto de la corrutina no cede frames de sobra). Esa rama de reutilización solo actualizaba el índice de línea y aplicaba el plano de apertura — **nunca volvía a calcular `_groupCamBaseDir` (dirección de cámara), `_cachedGroupCenter`/`_groupLookAtTarget` (centro/mirada del grupo) ni volvía a llamar a `PlayerParty.PositionMembersForGroupDialogue()`**, y ni siquiera actualizaba `_isGroupConversation` si el diálogo previo no era grupal. Resultado: el diálogo grupal heredaba el montaje calculado para la posición/sala del diálogo ANTERIOR al teletransporte — de ahí la cámara "en otro sitio", literalmente calculada para una sala que ya no es la actual. Coincide con el patrón "solo con Will no se veía" simplemente porque las cadenas `[Diálogo] → TeleportPlayer → [Diálogo grupal]` del contenido actual ocurren en tramos donde el personaje activo es Liam/Estela, no porque hubiera una rama de código específica de Will (esta causa es independiente del `HiddenNpc`). **Arreglado:** se extrajo el bloque de montaje grupal (elegir `_groupCamBaseDir` vía `PickBestGroupCameraDirection`, `PositionMembersForGroupDialogue`, recalcular centro/mirada del grupo, orientar NPC/player/compañeros) a un método nuevo `SetupGroupConversationStaging()`, reutilizado tanto en el montaje completo como en la rama de diálogo encadenado: esta última ahora actualiza `_isGroupConversation = isGroupConversation` y, si el resultado es grupal, vuelve a llamar a `SetupGroupConversationStaging()` antes de aplicar el plano — sin perder la ventaja de no apagar/reencender la cámara dedicada (evita el parpadeo que la reutilización existía para evitar). Cubre tanto el caso "individual→grupal encadenado" como "grupal→grupal encadenado tras moverse" (p.ej. un diálogo grupal partido en dos con un `TeleportPlayer`/`Move` entre medias), ya que ahora siempre se recalcula si el resultado final es grupal, sin depender de si el tipo cambió respecto al diálogo anterior.

  **Actualización (mismo día) — el fix de arriba no era la causa completa: Raúl confirmó que con Liam activo funciona, pero con Estela activa sigue rota.** Eso descarta la hipótesis "ocurre porque el contenido actual solo encadena teleport+grupal en tramos de Liam/Estela" — tiene que haber algo específico de Estela en ESTE tramo concreto. Log real de la repro con Estela activa: tras `[NarrativeExecutor:King] TeleportPlayer`, la cadena espera el evento `WE_HAVE_TO_SCAPE` — el propio comentario del nodo dice **"Evento que lanza Estela"**: el siguiente diálogo grupal lo dispara un `NPCInteractiveNarrativeExecutor` colgado del GameObject `_ESTELA` (la línea "hay que escapar de la prisión" la dice narrativamente el personaje Estela). `ExecuteDialogue()` pasa siempre `transform` — el del propio executor, es decir, el transform REAL de `_ESTELA` — como `npc` a `DialogueManager.StartDialogue`. Con Liam activo esto no pasa (ningún tramo de esta escena hace hablar a "Liam" como NPC), así que el bug nunca se manifestaba jugando con él; con Estela activa, el "NPC" de este diálogo grupal ES literalmente el propio personaje que el jugador está controlando. Y `_ESTELA` (el GameObject real) es exactamente `ActiveCharacterSwapper.HiddenNpc` en este momento: oculto, renderers apagados, `NavMeshAgent` congelado en la posición del último cambio de personaje — la misma "posición fantasma" que los fixes de agosto ya filtraban del CÁLCULO del centro de grupo, pero que nadie filtraba en el parámetro `npc` de entrada de `StartCinematic`/`DialogueManager.StartDialogue`. Confirmado además por el log: `[ActiveCharacterSwapper] NPC oculto '_ESTELA' tenía renderers reactivados fuera de la ventana de ReassertHiddenNextFrames — vuelto a ocultar por la red de seguridad periódica (0.5s). Bug 'dos Estelas y un Will' evitado` — el propio sistema de diálogo, al tratar a `_ESTELA` como "el NPC con quien se habla", intentaba mostrar sus renderers (comportamiento normal para cualquier NPC real), chocando de frente con `ActiveCharacterSwapper` manteniéndola oculta porque representa al personaje activo. **Arreglado (mismo commit):** en `DialogueCinematicController.StartCinematic()`, justo al entrar, si `npc == ActiveCharacterSwapper.Instance.HiddenNpc.transform` se redirige `npc = player` (el rig único, que es donde Estela vive de verdad ahora mismo) antes de cualquier otro cálculo — así todo el montaje grupal (que ya usa `SetupGroupConversationStaging()`, ver arriba) se calcula sobre la posición real y actual del personaje, no sobre el decoy congelado. Mismo redirect replicado en `DialogueManager.StartDialogue(DialogueAsset, Transform npc, Action)` para el `PositionMembersForDialogue(npc)` del camino no-grupal/no-cinemático (mismo problema, otra ruta). En ambos casos el redirect es solo para el transform de POSICIONAMIENTO/CÁMARA — `DialogueManager._currentNpc`/`_currentNpcDialogueCharacterId` (usados para nombre/portrait y para quien escuche `OnDialogueStarted`) se dejan intactos, apuntando al `_ESTELA` real, para no romper ninguna lógica de identidad que dependa de ellos. **Pendiente de confirmar por Raúl en juego** (ambos fixes, el de la reutilización de cadena y este del decoy-como-npc, quedan sin verificar en el Editor desde esta sesión).

- **INC-090 (20 ago 2026) — rocas del Golem (lanzadas y lluvia) sin daño de área ni VFX de impacto en el suelo — `GolemBossAI.cs` / `EnemyProjectile.cs`.** Reportado por Raúl: las rocas de la lluvia de Fase 2 deberían instanciar el VFX de grieta/aterrizaje (`shockwaveVFX` + `landingDustVFX`, vía `ConfigureGroundImpactVfx`) y hacer daño de área (`ConfigureAoE`) al tocar el suelo, pero no se ve el VFX ni se nota el daño; tampoco se nota daño de las rocas lanzadas directamente desde lejos (Fase 1/2). Incidencia de mucho tiempo sin resolver. **Causa raíz confirmada — velocidad real de la roca desacoplada de la configurada en el Golem:** `GolemBossAI.RockThrowAttack()`/`SetupRockProjectile()` calculan la dirección de tiro (incluida la predicción de movimiento del jugador, `PredictThrowTargetPosition`) usando `rockSpeed`/`rockRainSpeed` del Inspector (30 y 20 en el prefab actual del Golem, `Assets/Prefabs/Enemy/PBR_Golem.prefab`) y asignan `rb.linearVelocity` con esa velocidad — pero justo después llaman a `proj.Initialize(direction, -1f)`, y `EnemyProjectile.Initialize()` **pisaba** esa velocidad con `direction * speed`, donde `speed` es un campo serializado propio de `EnemyProjectile` fijo en el prefab de la roca (`Assets/Art/.../PBR_Rock.prefab`: `speed: 15`) — completamente ajeno al Inspector del Golem. `FixedUpdate()` reafirmaba esa misma velocidad equivocada en cada physics tick. Resultado: toda roca del Golem viaja siempre a 15 u/s pase lo que pase en el Inspector, y la predicción de tiro (calculada asumiendo 30 u/s para el lanzamiento directo) queda desfasada — el jugador se aleja del punto de impacto mucho más de lo previsto durante el tiempo de vuelo real (el doble de largo de lo calculado), lo que explica que las rocas lanzadas "no parezcan hacer daño" (fallan sistemáticamente contra un jugador en movimiento) — sin gravedad (`useGravity = false` en ambos tipos de tiro) el proyectil que falla vuela en línea recta y desaparece a los 5s sin tocar nada. Comprobado además: el suelo de la sala (`BattleArenaGolem` en `MainWorld.unity`, controlada por `BossArenaController`) usa el layer dedicado **"Floor"** (índice 10 de `TagManager.asset`) para el raycast de `PlaceBossOnFloor`, layer que `EnemyProjectile.OnTriggerEnter` no reconocía por nombre — solo caía en el fallback genérico `if (!other.isTrigger)` del final del método, que en teoría sigue funcionando pero sin dejarlo explícito en el código; los colisionadores de rocas (`PBR_Rock.prefab`) SÍ son trigger (`m_IsTrigger: 1`), lo que habilita `OnTriggerEnter` contra cualquier collider sólido sin depender de `OnCollisionEnter`, y la matriz de colisión (`DynamicsManager.asset`) no excluye ningún par de layers relevante (`ProjectileEnemy` colisiona con `Default`, `Floor` y `Player`), así que no se ha encontrado ninguna exclusión de físicas que bloquee el impacto por completo. **Arreglado:** `EnemyProjectile.cs` añade `SetSpeed(float)` (fija `_speedOverride`, usado por una nueva propiedad `EffectiveSpeed` en `Initialize()` y `FixedUpdate()` en vez del campo `speed` a secas) y `GolemBossAI.cs` lo llama justo antes de cada `Initialize()` (en `RockThrowAttack()` con `rockSpeed`, y en `SetupRockProjectile()` — compartido por Fase 2 y por `SpawnFallingRock` — con el parámetro `speed` recibido), de modo que la velocidad real coincide por fin con la usada para calcular la predicción de tiro. También se añade caché explícita de `_floorLayer` ("Floor") y se trata igual que `_defaultLayer` en el impacto de suelo, y se añaden `Debug.Log` de diagnóstico (bajo `#if UNITY_EDITOR || DEVELOPMENT_BUILD`) en cada rama de `OnTriggerEnter` que ignora un impacto sin manejarlo — incluyendo un log explícito si algún día el suelo de una sala resulta tener su collider marcado como `Is Trigger` (el único escenario en el que el fallback genérico NO dispararía VFX/AoE). **Pendiente de confirmar por Raúl en juego:** el fix de velocidad es la causa raíz más sólida y explica el patrón completo, pero si tras esta corrección las rocas de la lluvia siguen sin AoE/VFX, los nuevos logs de diagnóstico deberían señalar de inmediato si el suelo concreto de `BattleArenaGolem` tiene un layer o un `Is Trigger` inesperado.

- **INC-091 (23 ago 2026) — menú principal: al cerrar Ajustes/Controles, el resaltado visual salta a la opción de arriba en vez de quedarse en la que abrió el panel — `UISelectVisual.cs`.** Reportado por Raúl (con logs y capturas) como continuación directa del intento de fix documentado el 20/08 en la sección "Otros cambios" de la sesión de ese día, que **no funcionó**: el diagnóstico de entonces confirmó que `EventSystem.selected` es correcto (`BotonControles`) en todo momento, así que la causa tenía que estar en otra parte de la cadena de highlight visual. **Causa raíz confirmada hoy:** `MainMenuController.OnClickControls()`/`OnClickSettings()` desactivan `buttonPanel` mientras el panel está abierto y lo reactivan (`SetActive(true)`) en el callback de cierre, antes de restaurar la selección del botón que abrió el panel un frame después (`RestoreSelectionNextFrame`). Esa reactivación provoca que Unity reevalúe el raycast de hover ese mismo frame (o el siguiente) y dispare `OnPointerEnter` sobre lo que quede bajo el cursor del ratón **aunque el ratón lleve rato parado sin moverse** — es un hover "fantasma" causado por la reactivación de la jerarquía, no por movimiento real. `UISelectVisual.OnPointerEnter` ya tenía un fix del 20/08 que filtraba por familia de input activa (`InputGlyphService.CurrentFamily != KeyboardMouse → return`), pensado para que el ratón parado no pudiera robar la selección mientras se juega con mando — pero no cubría el caso real reportado: la acción "Cancel" que cierra el panel está ligada tanto a `<Gamepad>/buttonEast` como a `<Keyboard>/escape` (`PlayerControls.inputactions`), así que cerrar el panel con Esc no cambia la familia activa — se sigue "en" KeyboardMouse, y el filtro de familia no bloqueaba nada. El hover fantasma dispara `EventSystem.SetSelectedGameObject(otroBotón)` sobre lo que sea que quede bajo el cursor (p.ej. Configuración, justo encima de Controles en el layout), robándole la selección visual al botón recién restaurado. **Arreglado:** `UISelectVisual.OnPointerEnter` ahora, además del filtro de familia existente, exige que el ratón se haya movido de verdad este frame (`Mouse.current.delta.ReadValue().sqrMagnitude` por encima de un umbral — mismo criterio y mismo umbral, `4f`, que ya usa `InputGlyphService` para decidir si hay actividad real de ratón) antes de tratar el evento como hover legítimo y robar la selección; un hover fantasma disparado por la reactivación del panel siempre trae delta ≈ 0. **Pendiente de confirmar por Raúl en juego** (no se ha podido reproducir en el Editor desde esta sesión) — si el salto de resaltado sigue apareciendo con este fix puesto, revisar además la hipótesis alternativa ya apuntada el 20/08: un `ScrollRect`/layout que reposicione la vista al primer elemento visible.


- **INC-092 (23 ago 2026) — el hint de "Teletransporte" del HUD se quedó con el estilo antiguo tras el rediseño de interfaz — `TeleportHintUI.cs` (sprite en `Start.unity`, sin tocar código).** Reportado por Raúl con captura: la burbuja "Teletransporte" (esquina superior derecha, aparece junto a los puntos de guardado) seguía con el marco dorado ornamentado de antes del rediseño de interfaz, mientras el resto del HUD (barra de HP/MP, el prompt de interacción "E", los paneles de Misiones) ya usa el estilo "glass" nuevo (fondo azul/morado oscuro con estrellitas y borde degradado). **Causa raíz:** el fondo (`Image` del GameObject `hintRoot` en `TeleportHintUI`, dentro de `Start.unity`) seguía apuntando a `Assets/Art/UI/Misiones/title_BG.png` (guid `6ffa3947f40f24c4f9a15227ed65b513`, del 3 de mayo, marco dorado con florituras — el mismo estilo que tenían los paneles de Misiones antes del rediseño del 13 de agosto). El resto de fondos de Misiones sí se sustituyeron ese día por sus equivalentes "glass" (`panel_glass_bg.png`, `header_pill_bg.png`, `state_chip_*.png`, etc.) pero `title_BG.png` quedó huérfano — sin ningún otro sitio que lo siga usando en el proyecto — y nadie actualizó la referencia del hint de teletransporte al mismo tiempo. **Arreglado:** se cambió la referencia de sprite de ese único `Image` a `Assets/Art/UI/Misiones/header_pill_bg.png` (guid `cf70966e009f4a8181df8ac98c68a807`) — el sprite "glass" con forma de píldora ya creado en el mismo rediseño del 13/08 para cabeceras, y que además ya es el fondo del propio HUD de Misiones en esta misma escena — y se cambió el `Image.Type` de `Simple` a `Sliced` (`m_Type: 0` → `1`) para que respete el 9-slice (`spriteBorder: 46,46,46,46`) ya definido en el importer del sprite, igual que en sus otros dos usos en `Start.unity` (ambos también `Sliced`). No se tocó ningún script ni el `buttonIcon` (el icono del botón "T"/mando ya usa iconografía nueva, gestionada aparte por `InteractionHintIconSet` según el dispositivo activo — no formaba parte del problema). **Cómo se aplicó:** edición puntual de dos valores ya serializados en `Start.unity` (el guid del sprite y el entero de `Image.Type`), verificada con `git diff` para confirmar que solo cambiaron esas dos líneas — no se creó ningún GameObject nuevo ni se tocaron referencias internas de prefab, así que no hizo falta pasar por una herramienta de Editor con `[MenuItem]` como en INC-091/Garra del Pacto. **Pendiente de confirmar por Raúl en juego:** abrir el proyecto en Unity, comprobar que la burbuja de Teletransporte se ve con el mismo estilo "glass" que el resto del HUD (sin distorsión en las esquinas redondeadas — el 9-slice debería encargarse de eso) y hacer commit del `.unity` si todo está bien.

- **INC-095 (25 ago 2026) — Will (NPC) no camina ni anima; se queda quieto y "salta" cerca del jugador — `NPCBehaviourManagerV2.cs` / `NPCStateContext.cs` / `FollowPlayerState.cs`.** Reportado por Raúl: controlando a Estela (o cualquier personaje activo distinto de Will), el NPC de Will no caminaba ni reproducía animaciones de movimiento — se quedaba quieto y de repente "saltaba": desaparecía y reaparecía más cerca del jugador. El mismo síntoma podía darse con cualquier compañero NPC, no solo Will (el reporte menciona explícitamente "ya sea Will o Estela"). Se esperaba el mismo comportamiento fluido que tienen Liam/Estela cuando son NPCs con Will de jugador. **Causa raíz:** `HasActiveAiExemption()` en `NPCBehaviourManagerV2` decide qué NPCs quedan exentos del sistema de sueño por distancia (LOD de IA): combate, cinemática, interacción, refugio de lluvia, o `IsAlly` (`_cachedPartyMember.IsInParty`). El NPC de Will (`WillNpcInstance`, instanciado por `ActiveCharacterSwapper` cuando Will no es el personaje activo) es, a propósito, un compañero que sigue al jugador SIN pasar nunca por `JoinParty()`/`PlayerParty` — hay un comentario explícito en `DialogueCinematicController.cs` confirmando que esto es intencional. `IsAlly` era por tanto siempre `false` para el NPC de Will, y en cuanto el jugador se alejaba más de `aiUpdateDistance` (45m) + margen, `EnterFarState()` lo dormía: dejaba de correr su `Update()` de movimiento/animación mientras seguía existiendo en la escena. El único sistema que lo "despertaba" era el teletransporte de emergencia por distancia de `PlayerParty.CheckMemberDistances()` (cada 0.3s) — de ahí el patrón exacto reportado: quieto, sin animar, y "saltando" cerca del jugador cada vez que la distancia de emergencia se disparaba, en vez de caminar con normalidad como Liam/Estela (sí registrados en el party real, por tanto exentos). **Arreglado, de forma genérica y no solo para Will:** nueva propiedad `NPCStateContext.IsActivelyFollowingPlayer` (`NPCStateContext.cs:43`), puesta a `true` en `FollowPlayerState.OnEnter` (`FollowPlayerState.cs:186`) y a `false` en `OnExit` (`:922`) — "true mientras el NPC esté de verdad en el estado de seguir al jugador", sin importar si llegó ahí por ser miembro real del party o por el mecanismo paralelo del NPC de Will. `HasActiveAiExemption()` (`NPCBehaviourManagerV2.cs:380`) añade este flag como condición de exención adicional, junto a `IsAlly`. Con esto cualquier NPC activamente siguiendo al jugador queda exento de dormir por LOD de distancia, sea o no miembro registrado del party — cubre el caso de Will hoy y cualquier otro "compañero temporal" similar que se añada en el futuro sin tener que volver a tocar `HasActiveAiExemption()`. **Pendiente de confirmar por Raúl en juego.**

  **Actualización (mismo día) — KO en juego: Raúl confirma con logs que Will sigue sin caminar.** El heartbeat de diagnóstico (`ActiveCharacterSwapper`, 🫀) muestra `estado=FollowPlayer` sostenido y `onNavMesh=True`, pero la posición de Will (`pos=(0.22, 3.50, -77.92)`) permanece IDÉNTICA durante varios latidos seguidos (varios segundos reales, con la cámara moviéndose con normalidad — `distCam` pasa de 4,4m a 27,1m sin que Will se mueva ni un centímetro), hasta que `PlayerParty` dispara el teletransporte de emergencia ("Will NPC demasiado lejos (26,2m > 25,0m), teletransportando...") y Will vuelve a quedarse clavado en el punto nuevo. El fix de arriba (exención de LOD) SÍ funcionaba — el NPC ya no se dormía, se quedaba en `FollowPlayerState` de forma continua — pero eso solo puso al descubierto un segundo bug, hasta ahora enmascarado por el primero: el propio cálculo de destino del NavMeshAgent estaba roto de raíz para cualquier "compañero temporal" no registrado en `PlayerParty`. **Causa raíz real:** `FollowPlayerState.UpdateDestination()` calcula el punto al que debe caminar el NPC con `_partyMember.GetFormationPosition()` (cuando `preferBehind` y hay `_partyMember`, el caso normal). `NPCPartyMember.GetFormationPosition()` (línea 694) era `return _party?.GetFormationPosition(PartyIndex) ?? transform.position;` — y `_party` (la referencia a `PlayerParty`) solo se asigna dentro de `JoinParty()`/similares, nunca para el NPC de Will (que, a propósito, no pasa por ahí — ver INC-095 arriba). Con `_party` siempre `null` para Will, el operador `??` caía siempre en el fallback: `transform.position`, **la propia posición ACTUAL de este mismo NPC**. `UpdateDestination()` llamaba a `context.Agent.SetDestination(hit.position)` con ese punto — es decir, se le ordenaba al `NavMeshAgent` de Will caminar hacia donde ya estaba parado. `Agent.isStopped = false` y `Agent.speed` se fijaban con normalidad (por eso `estado=FollowPlayer` y `onNavMesh=True` parecían correctos en el heartbeat), pero al no haber ningún desplazamiento real que recorrer, la velocidad del agente se quedaba en cero — y como `UpdateMovementAnimation()` (`NPCStateBase.cs:50`, compartido por todos los estados de NPC) deriva el parámetro de animación de `NavMeshAgentUtility.ComputeSpeedFactor(context.Agent)` (la velocidad REAL del agente, no un booleano de intención), Will tampoco animaba: exactamente el síntoma original reportado ("no camina ni hace las animaciones"), ahora con la causa de fondo correcta — nunca fue un problema de animación ni de decisión de moverse, sino un destino de navegación degenerado. Liam/Estela nunca mostraban este bug porque sí pasan por `JoinParty()`, así que su `_party` nunca es `null` y `GetFormationPosition()` siempre devuelve un punto real detrás del jugador. **Arreglado — en el método de origen, no solo en el punto de llamada, para que cualquier consumidor futuro quede cubierto igual:** `NPCPartyMember.GetFormationPosition()` ya no usa `transform.position` como fallback. Cuando `_party` es `null`, calcula un punto justo detrás del jugador (`_npcManager.Context.Player.position - player.forward * (distanciaParaPararse * 0.8)` — mismo criterio que ya usa `FollowPlayerState.GetFormationTarget3D()` para el seguimiento especial, sin spread lateral por índice porque aquí no hay `PartyIndex` válido) en vez de la propia posición del NPC, que nunca es un destino de seguimiento válido. Revisado el único otro consumidor de este método (`NPCPartyMember.OnDrawGizmosSelected()`, gizmo de editor) — está detrás de un guard `!_isInParty) return`, así que nunca llega a este camino y no se ve afectado. **Confirmado por Raúl en juego (25 ago 2026): incidencia dada por solucionada.**

- **INC-096 (25 ago 2026) — miembros del party se quedan atrás al esprintar y "hacen pop-in" cerca del jugador (cabezas asomando por la parte baja de la cámara) — `FollowPlayerState.cs` / `ActiveCharacterSwapper.cs` / `Estela_PartyConfig.asset` / `Will_NPC_PartyConfig.asset`.** Reportado por Raúl: al esprintar, los compañeros del party se quedan atrás, se van teletransportando cerca del jugador, y ese teletransporte se nota en cámara — "se ve como desaparece y aparecen cabezas por la parte baja de la pantalla". **Causa raíz:** la velocidad de sprint del jugador está fija en el prefab (`_WILL.prefab`: `walkSpeed 4`, `runningSpeed 6`, `sprintSpeed 15` de `freeSpeed`, con `sprintSpeedMultiplier 1.3`), muy por encima de la velocidad de correr configurada para los NPCs del party (`velocidadCorriendo: 8` en `Estela_PartyConfig.asset` y `Will_NPC_PartyConfig.asset`, frente a `10` que ya tenían Liam/Eldran) — un NPC nunca podía alcanzar al jugador esprintando, se quedaba cada vez más atrás hasta que `PlayerParty.CheckMemberDistances()` disparaba el teletransporte de emergencia (`distanciaParaTeletransporte: 25` en las cuatro configs) — un warp brusco, visible en cámara si el jugador gira o mira atrás. **Arreglado, en dos capas — dato de config y sistema genérico:** (1) `velocidadCorriendo` de `Estela_PartyConfig.asset` y `Will_NPC_PartyConfig.asset` subida de 8 a 10, igualada a Liam/Eldran — reduce cuánto se queda atrás un NPC en sprint, pero no lo resuelve del todo porque el jugador sigue siendo más rápido en sprint que cualquier velocidad de config fija. (2) Sistémico: en `FollowPlayerState.cs` se sustituyó la velocidad de "correr hacia el jugador" fija (`runSpeed` de la config) por una velocidad dinámica que sigue la velocidad real y reciente del propio jugador. `UpdatePlayerSpeedEstimate()` (línea 834) mide la velocidad instantánea del jugador cada frame (descartando picos por encima de `PLAYER_SPEED_TELEPORT_CUTOFF = 30`, que delatan un teletransporte del jugador y no movimiento real) y la suaviza con `Mathf.Lerp` a razón de `PLAYER_SPEED_SMOOTH_RATE = 6`/s en `_smoothedPlayerSpeed`. `dynamicRunSpeed` (línea 343) se calcula como `_smoothedPlayerSpeed * 1.15` (15% de margen para poder cerrar distancia, no solo igualar), acotado entre `runSpeed` y `runSpeed * 2.5` para no dispararse ni quedarse corto; el mismo cálculo (`baseSpecialSpeed`, línea 462) se aplica al movimiento "especial" (nado/vuelo/fuera de NavMesh) en `HandleSpecialFollow`. Con esto un NPC siguiendo al jugador en sprint persigue a una velocidad proporcional al sprint real en vez de a una constante de config que nunca alcanza al jugador a fondo — reduce drásticamente cuántas veces se dispara el teletransporte de emergencia (y por tanto el pop-in en cámara), sin importar qué tan rápido corra el jugador en el futuro, porque se adapta solo. (3) Además, para cuando el teletransporte de emergencia sí ocurre (sprint en línea recta muy larga, o NPC bloqueado): se cambió el punto de destino del NPC de Will en `ActiveCharacterSwapper.TeleportWillNpcToPlayer()` (línea 553) de un punto fijo "1.5m detrás del jugador" (que amontonaba a varios NPCs en el mismo punto, agravando el "aparecen cabezas" si hay más de un compañero) a `PlayerParty.GetFormationPosition(slotIndex)` (línea 574) — el mismo cálculo de posición en semicírculo detrás del jugador que ya usan los miembros reales del party, así que el NPC de Will aparece en su propio hueco de formación en vez de superpuesto con los demás. **Pendiente de confirmar por Raúl en juego.**

  **Actualización (mismo día) — KO en juego: Raúl reporta que le siguen "apareciendo por la izquierda" (pop-in visible).** Log real: `[PlayerParty] ⚡ Liam demasiado lejos (25,7m > 25,0m), teletransportando...` y `⚡ Estela demasiado lejos (26,9m > 25,0m)...` repitiéndose cada pocos segundos durante un tramo de sprint sostenido — es decir, el teletransporte de emergencia seguía disparándose una y otra vez, con la distancia oscilando justo por encima del umbral (25,1–26,9m) en vez de mantenerse estable por debajo. El fix de arriba SÍ aplicaba correctamente (`dynamicRunSpeed` se calculaba y se asignaba a `Agent.speed` cada frame), pero el margen era un 15% FIJO sobre la velocidad estimada del jugador — en la práctica insuficiente para cerrar el hueco de verdad: el suavizado de `_smoothedPlayerSpeed` tarda un rato en converger tras cada arranque de sprint (mientras tanto el NPC va más lento de lo que el 15% sugiere), y el camino que sigue el `NavMeshAgent` por NavMesh casi siempre rodea más metros que la línea recta con la que `PlayerParty` mide la distancia para decidir el teletransporte — así que un margen de velocidad fijo, en el mejor de los casos, dejaba al NPC EMPATANDO con el jugador en vez de ganándole terreno de verdad, y la distancia se quedaba oscilando alrededor del umbral de teletransporte en lugar de reducirse. **Arreglado:** el margen de velocidad ya no es un 15% constante — crece con la propia distancia actual, igual que ya hacía (y sigue haciendo) `HandleSpecialFollow()` para nado/vuelo (`baseSpecialSpeed * 1.5` cuando `dist > SPECIAL_FOLLOW_STOP_DIST * 3`), patrón que nunca se había traído al seguimiento normal por NavMesh hasta ahora. `catchUpT` interpola de 0 (a `distanciaParaCorrer`) a 1 (al propio umbral `distanciaParaTeletransporte` de este NPC, `_config?.distanciaParaTeletransporte ?? 25f`), y `speedMargin` va de 1.15 a 1.6 según ese valor — así que cuanto más se acerca el NPC al punto de disparar el teletransporte de emergencia, más agresivamente intenta cerrar el hueco por sí solo, en vez de solo mantener el ritmo del jugador. **Pendiente de confirmar por Raúl en juego** — con este segundo ajuste, la distancia debería reducirse de forma sostenida durante un sprint largo en vez de oscilar justo bajo el umbral, y los teletransportes de emergencia (y el pop-in visible que los delata) deberían quedar solo para casos extremos (sprint en línea recta muy larga sin ningún momento de respiro, o el NPC bloqueado por geometría).

  **Actualización (1 sept 2026) — KO en juego otra vez: Raúl confirma "trompicones... saltos en la cámara y de pronto aparecen... rompe la experiencia de sprintar".** El segundo ajuste (margen de velocidad creciente) sí hacía que `dynamicRunSpeed` pidiera hasta 2.5x `runSpeed` (hasta 25 con `velocidadCorriendo: 10`), pero `FollowPlayerState` nunca tocaba `Agent.acceleration` — se quedaba en el valor serializado de fábrica en el prefab (`m_Acceleration: 8` en `_ESTELA.prefab`/`_LIAM.prefab`), sin relación con la velocidad objetivo que se le pedía. Con el destino recalculándose cada `PATH_UPDATE_INTERVAL` (0.1s) mientras el jugador sigue esprintando, el `NavMeshAgent` nunca llegaba a acelerar lo suficiente para alcanzar de verdad esa velocidad alta — en la práctica seguía rindiendo muy por debajo de los 25 teóricos, así que la distancia seguía creciendo hasta cruzar `distanciaParaTeletransporte` (25m) y disparar el pop-in, el mismo síntoma que la actualización anterior. **Arreglado:** nuevo helper `FollowPlayerState.SetAgentSpeed()` que fija `Agent.speed` y escala `Agent.acceleration` junto con ella (`Mathf.Max(8, speed * 2.5)`), sustituyendo las tres asignaciones directas de `Agent.speed` que había en el archivo (entrada al estado, seguimiento normal/catch-up, vagabundeo cerca del jugador) — así el agente puede ramp-up de verdad hasta la velocidad de catch-up en vez de quedarse limitado por una aceleración fija pensada para el paso normal. **Pendiente de confirmar por Raúl en juego** (tercer intento) — con este ajuste la distancia debería cerrarse de forma real durante un sprint sostenido, no solo teóricamente vía `Agent.speed`.

- **INC-097 (25 ago 2026) — miembros del party caminan "a trompicones"/temblando a baja velocidad (nadando, o con la velocidad reducida por el pop-up de lore al seguir a un guardia) — `FollowPlayerState.cs`.** Comparte archivo, sesión y commit con INC-096 — se documenta aparte por ser un síntoma distinto (temblor/arranca-para, no teletransporte) del mismo problema de fondo: velocidad de seguimiento desacoplada de la velocidad real del jugador. Reportado por Raúl: al nadar, o cuando la velocidad del jugador baja (por ejemplo con el pop-up de lore abierto mientras hay que seguir a un guardia despacio), los miembros del party caminan a trompicones, se ven temblando. **Causa raíz:** la decisión de moverse o pararse en `OnUpdate()` se basaba en `playerIsMoving` (un booleano sin memoria entre frames) `|| distance > stopDist * 1.2` — sin ninguna histéresis. Cuando el jugador se mueve lento y de forma entrecortada (nado a baja velocidad, o el paso corto de un guardia con el pop-up de lore abierto), el NPC entraba y salía del estado "moviéndose" varias veces por segundo — cada transición para/arranca la animación del `Animator` y reinicia el `NavMeshAgent` (`isStopped` a golpes), lo que se percibe como temblor/trompicones. **Arreglado:** nuevo campo `_isFollowingSticky` (línea 43) implementa histéresis de "pegajosidad": una vez que el NPC empieza a moverse (`_isFollowingSticky = true`) se mantiene moviéndose hasta llegar a distancia de parada real (`distance <= stopDist`, donde se resetea explícitamente en línea 300), en vez de poder parar y arrancar en cada frame que `playerIsMoving` oscile. La decisión (línea 325) queda `_isFollowingSticky ? true : (playerIsMoving || distance > stopDist * 1.2)` — solo se reevalúa desde parado. Combinado con la velocidad dinámica de INC-096 (que ahora sigue la velocidad real y suavizada del jugador en vez de un `runSpeed`/`walkSpeed` fijo, así que a velocidad de nado lenta el NPC también camina lento y de forma continua, no a trompicones entre "quieto" y "corriendo a full"), el NPC mantiene un movimiento fluido tanto si el jugador va rápido como si va despacio o de forma entrecortada. **Pendiente de confirmar por Raúl en juego.**

  **Actualización (1 sept 2026) — KO en juego: Raúl confirma trompicones también corriendo, no solo a baja velocidad.** Causa raíz adicional, no cubierta por el fix de histéresis de arriba: `NPCSimpleAnimator` normaliza el parámetro de movimiento del `Animator` (el blend walk/run) en `SyncWithNavMeshAgent()` usando `_walkSpeed`/`_runSpeed`, que se leían de `behaviourMgr.Configuration.walkSpeed`/`.runSpeed` (`NPCConfiguration`, pensada para NPCs ambientales genéricos) — **1.5 y 4** en los prefabs de Estela/Liam, verificado directamente en `_ESTELA.prefab`/`_LIAM.prefab`. Esos valores nunca se sincronizaron con las velocidades reales que `FollowPlayerState` usa de verdad para mover a un compañero (`NPCPartyConfig.velocidadCaminando`/`.velocidadCorriendo` = 5/10, y hasta 25 durante el catch-up dinámico de INC-096). Como `SyncWithNavMeshAgent()` calcula `normalizedSpeed = 0.5 + (agentSpeed - _walkSpeed) / (_runSpeed - _walkSpeed) * 0.5` y lo pasa por `Mathf.Clamp01`, cualquier velocidad real por encima de 4 — que es prácticamente cualquier movimiento del compañero, incluso al paso normal — se clampaba de inmediato a `normalizedSpeed = 1.0`: la animación de piernas se queda pegada al ritmo fijo de "correr al máximo" mientras el cuerpo se desliza por el NavMesh a la velocidad real, mucho más alta y variable — un patinazo/desacople visual (no un problema de arranca-para como el de arriba) que explica por qué el fix de histéresis, correcto para su síntoma, no bastaba. **Arreglado:** en `NPCSimpleAnimator`, al inicializar `_walkSpeed`/`_runSpeed` se comprueba primero si el NPC tiene `partyConfig` asignado (`behaviourMgr.Configuration.partyConfig`) y, si lo tiene, se usan sus velocidades reales de seguimiento (`partyCfg.walkSpeed`/`.runSpeed`, propiedades de compatibilidad que ya exponen `velocidadCaminando`/`velocidadCorriendo`) en vez de las genéricas de `Configuration` — sin tocar el comportamiento de NPCs ambientales sin `partyConfig`, que siguen usando los valores de siempre. **Pendiente de confirmar por Raúl en juego** (segundo intento) — el blend debería distinguir ahora entre paso normal (~5) y sprint de alcance (hasta 25, clampado igualmente en el extremo superior, pero ya no desde el primer paso).

- **INC-098 (25 ago 2026) — nubes ambientales demasiado bajas tapan la cámara en diálogos; salen pocas nubes de golpe — `AmbientCloudDirector.cs` / `DialogueCinematicController.cs` / `MainWorld.unity` / `Sendero.unity`.** Reportado por Raúl: las nubes sueltas (distinto del techo de tormenta de `CloudCoverSpawner`, ya arreglado en la sesión anterior — ver `fix-nubes-invisibles-y-techo-bajo-2026-08-24` en el histórico de sesiones) están mucho mejor, pero a veces bajan tanto que tapan la cámara durante los diálogos; deberían estar siempre por encima del jugador con margen de altura suficiente para no tapar la cámara, pero sin dejar de verse — le gustan las que sí salen. También le gustaría que salieran más nubes de golpe a veces, hoy solo 1-2 sueltas. **Causa raíz, dos partes independientes:** (1) Altura mínima insuficiente: `MainWorld.unity` y `Sendero.unity` tenían overrides de escena para `AmbientCloudDirector` con `altitudeJitter: 15` (por encima incluso del default de 8 del propio script) y `minCloudAltitude: 20` — con ese jitter, una pasada de nube podía bajar hasta `minCloudAltitude - altitudeJitter` = 5m sobre el jugador, de sobra para cortar la cámara de diálogo (que suele mirar desde más cerca/arriba que la cámara de exploración libre). (2) Sin mecanismo de ocultación durante diálogos: el propio script ya tenía un método público `SetAmbientCloudsVisible(bool)` (`AmbientCloudDirector.cs:290`), pensado exactamente para este caso (ocultar nubes durante un encuadre puntual, como ya hace `FocusCameraNode` con otros elementos) — pero sin ningún llamador: grep confirmó cero referencias a esa llamada en todo el proyecto antes de este fix. (3) Pocas nubes a la vez: `clusterChance: 0.3` y `clusterExtraMax: 2` en las escenas hacían que la mayoría de spawns fueran una nube suelta, como mucho 2 extra en un cluster. **Arreglado:** altura — `minCloudAltitude` subido de 20 a 32 (default del script y override de `MainWorld.unity`; `Sendero.unity` no tenía override propio, así que hereda el nuevo default) y `altitudeJitter` bajado de 15 a 8 (default del script, `MainWorld.unity` y `Sendero.unity`) — deja un suelo real de 24m sobre el jugador en el peor caso, con margen de sobra para cualquier cámara de diálogo sin dejar de estar "bastante" por encima como pidió Raúl. Frecuencia/cantidad — `clusterChance` subido de 0.3 a 0.5 y `clusterExtraMax` de 2 a 4 en ambas escenas (más probabilidad de que salgan varias nubes juntas, hasta 4 extra por cluster), y `poolSize` subido de 6 a 10 para que el pool de instancias no sea cuello de botella al pedir más nubes simultáneas. Ocultación durante diálogos — se añadió el patrón singleton obligatorio (CLAUDE.md § 3) a `AmbientCloudDirector` (`Instance` + `ResetStatics` bajo `RuntimeInitializeOnLoadMethod.SubsystemRegistration`, líneas 47-51) para que otros sistemas puedan pedir `SetAmbientCloudsVisible` sin buscar/cachear el componente. `DialogueCinematicController.StartCinematic()` llama a `AmbientCloudDirector.Instance?.SetAmbientCloudsVisible(false)` al entrar en modo cinemático de diálogo (línea 653), `EndCinematicImmediate()` lo revierte a `true` (línea 421), y `OnDestroy()` incluye una red de seguridad que también lo revierte a `true` si el objeto se destruye a mitad de un diálogo (línea 881) — para que las nubes nunca queden ocultas de forma permanente si el diálogo termina de forma anómala. Con la altura ya corregida esto es cinturón-y-tirantes (las nubes ya no deberían necesitar ocultarse en el caso normal), pero cubre encuadres de cámara muy cerrados o ángulos que la subida de altura no llegue a prever. **Confirmado por Raúl en juego (25 ago 2026): incidencia dada por solucionada.**

- **INC-099 (25 ago 2026) — techo de nubes de tormenta demasiado bajo al llover, se ve raro — `CloudCoverSpawner.cs`.** Reportado por Raúl con captura (lluvia sobre el pueblo, montañas nevadas al fondo): a diferencia de INC-098 (nubes ambientales sueltas, sistema distinto, ya resuelto en esta misma sesión), aquí se trata del techo de tormenta real que anuncia la lluvia — el jugador sigue viendo el techo de nubes demasiado bajo y con aspecto raro incluso tras el fix del 24 ago de `CheckRecenter()` (fix-nubes-invisibles-y-techo-bajo-2026-08-24), que arregló el recentrado en desnivel de terreno pero no tocó la altura base. **No es un bug nuevo, es un ajuste puramente estético de un valor ya tocado varias veces (60→90 el 23 ago) sin llegar a "verse bien" del todo — sin acceso al Editor no se puede comprobar visualmente, así que este fix es una subida directa de `cloudHeight` a ojo, pendiente de que Raúl confirme si hace falta subir más.** **Cambiado:** `cloudHeight` 90→130 (altura base del techo sobre el jugador), `heightJitter` 12→16 y `arrivalExtraHeight` 50→65 en la misma proporción, para que la variación aleatoria de altura y la caída de llegada de cada nube seguían encajando con la nueva altura base en vez de quedarse chicas en comparación. `horizonElevationDegrees` (30°, controla cuánto suben las nubes lejanas) se deja igual — no es la causa aquí, ese fix del 20 ago ya resuelve el ángulo de elevación en horizontal, esto es la altura base plana cerca del centro del techo. **Pendiente de confirmar por Raúl en juego** — si las nubes se siguen viendo bajas/raras, seguir subiendo `cloudHeight` (y los otros dos en proporción) es directo, ya documentado en el tooltip del campo.

- **INC-103 (25 ago 2026, PENDIENTE — no arreglado a petición de Raúl) — persistencia de interactuables del castillo (puzle de interruptores/arañas + puertas de entrada) — `ActivationCounter.cs` / `Door.cs` / `CastleDoorController.cs` / `DoorGate.cs` / `MainWorld.unity`.** Reportado por Raúl cargando una partida desde una build: en el castillo (zona del anchor `CastleInterior1`) hay un puzle de interruptores con arañas — conseguir la llave abre una puerta, y las puertas que se van abriendo durante el propio puzle también deberían quedar así para siempre. Al recargar, ese estado no persiste. Por separado, de las dos puertas de la entrada principal del castillo (que deberían estar ambas abiertas a estas alturas de la partida) una aparecía cerrada y la otra encajada dentro del muro. Raúl pidió explícitamente NO resolver esto "game object a game object" con una solución mala, sino entender primero cómo abordarlo — a diferencia de los NPCs (persistencia vía FSM/checks dedicados), no tenía claro qué patrón aplica aquí. Esta entrada documenta la investigación, sin aplicar ningún fix.

  **Puzle de interruptores — causa raíz confirmada leyendo `MainWorld.unity` directamente.** El proyecto ya tiene un mecanismo de persistencia por instancia para este tipo de puzles: `ActivationCounter` (`Assets/Scripts/Puzzle/ActivationCounter.cs`) expone un flag opt-in `persistOnComplete` (default `false`) que, al activarse, guarda `ACTIVATION_COMPLETE_{id}` en `preset.flags` cuando se completa (`RegisterActivation()` → `SetFlag()`) y lo restaura en `OnProfileReady`/`OnPresetApplied` (`TryRestoreFromPreset()`, re-invoca `onRequirementMet` para que la puerta vuelva a abrirse). El puzle de la escena usa 8 pares `ActivationCounter`+`Door` (localizados por GUID de script: `cd885445e1328904eb846ba210b2d668` / `653b23a30cde07c49b1d1357f14609eb`), la mayoría con `requiredCount: 1` (interruptores individuales) y uno con `requiredCount: 3, lockOnComplete: true` (probablemente el gate de la llave). De los 8, **solo 1 tiene `persistOnComplete: 1`** — los otros 7 lo tienen en `false` (el default de fábrica del componente, sin tocar en el Inspector). Como `Door.cs` no tiene persistencia propia ni método de apertura instantánea (solo `Open()`/`Close()` animados, llamados desde `ActivationCounter.onRequirementMet` vía `UnityEvent`) y depende por completo de que su `ActivationCounter` vuelva a invocar el evento al cargar, los 7 interruptores sin `persistOnComplete` nunca reabren su puerta tras un reload — vuelven al estado inicial serializado en la escena (cerradas, `startOpen: 0` en los 8). Coincide exactamente con el síntoma reportado.

  **Puertas de la entrada principal del castillo — sistema de persistencia correcto, sin defecto encontrado por análisis estático.** Hay un único `CastleDoorController` en la escena (GameObject `CLOSE_THE_DOORS_TRIGGER`, bajo `EVENTS`), con `persistState: 1` y `persistenceId: MainCastleGate` — guarda/restaura `DOOR_OPEN_MainCastleGate` en `preset.flags` igual que `ActivationCounter`, y a diferencia de `Door.cs`, `DoorGate.cs` sí tiene `OpenInstant()` (usado por `CastleDoorController.ApplyOpenInstant()` al restaurar, sin animación). Las dos puertas (`leftDoor`/`rightDoor`) son instancias de prefab con `openAngle: 90` y `openAngle: -90` respectivamente — correctamente espejadas — y ambas `startsOpen: 0`. El cierre automático al entrar (`CastleDoorCloseTrigger.cs`) tiene un `questGate` configurado (`mode: QuestStartedOrCompleted` sobre `Q_ELDRAN_MISSION13.asset`, la misión de `GDD.md` cuyo fin dispara "Reabrir puertas del castillo") que, en el punto de la partida donde el log de Raúl muestra al grafo narrativo esperando activamente esa misión (`WaitQuestCompleteNode 'Espero a completar la mision 13'`), debería estar `Active` y por tanto bloquear cualquier recierre. No se ha encontrado ninguna condición de carrera ni dato mal serializado que explique el síntoma (puerta cerrada + puerta encajada en el muro) solo leyendo escena y código — **pendiente de reproducir en el Editor** para diagnosticar si es un problema de timing en runtime (activación/desactivación del `CastleDoorController` por algún sistema de LOD/culling que no se ha localizado), de la lógica narrativa que dispara `OpenDoors()` (`CastleNarrativeEventListener.cs`, escucha un evento custom una sola vez — si ese evento nunca llegó a dispararse en esta partida concreta, el flag `DOOR_OPEN_MainCastleGate` nunca se habría guardado y ambas puertas deberían verse cerradas, no solo una — inconsistente con lo reportado y a revisar), o de otra causa no identificada todavía.

  **Pregunta abierta para la próxima sesión (no resuelta aquí a petición de Raúl):** el proyecto no tiene hoy un patrón único de "persistencia de estado de mundo" — cada sistema lo resuelve por su cuenta (flags en `preset.flags` opt-in por instancia en `ActivationCounter`/`CastleDoorController`, listas dedicadas para bosses en `BossProgressTracker`, checks en la FSM para NPCs). Para este puzle en concreto el mecanismo ya existe y funciona (`persistOnComplete`) — el bug real es que es opt-in y fácil de olvidar por instancia (7 de 8 aquí). Antes de tocar interruptor a interruptor, decidir si conviene: (a) invertir el default a persistente-por-defecto (`persistOnComplete = true` salvo que se desmarque a propósito) para que un puzle nuevo persista sin acordarse de marcarlo, (b) una herramienta de validación de escena (mismo patrón que `El Sendero/Narrativa/Validar Interactive vs Grafo`) que avise de `ActivationCounter`/`CastleDoorController` sin persistencia activada antes de una entrega, o (c) mantener el opt-in actual pero documentar mejor la convención. Ninguna de las tres se ha implementado.

- **INC-148 (1 sept 2026) — la animación de vuelo "se quita y se pone" en bucle, tanto del jugador como de los compañeros de party que vuelan con él — `PlayerFlyingController.cs` / `FollowPlayerState.cs`.** Reportado por Raúl en el mismo mensaje que INC-096/INC-097. **Causa raíz:** `PlayerFlyingController.Update()` llama a `IsGrounded()` (un `Physics.CheckCapsule` contra el propio collider del jugador, con `GroundCheckBuffer = 0.2f`) en cada frame mientras `_isFlying` es `true`, y en cuanto da `true` una sola vez llama a `ExitFlight()` de inmediato — sin ningún margen. Volar a baja altura o rozar el terreno es parte normal del mecanismo (picados con `fly_dive`, vuelo rasante), así que un único frame de solape con el suelo bastaba para disparar una salida de vuelo real: `ExitFlight()` desactiva el `vThirdPersonController`, restaura la gravedad del `Rigidbody` y hace `PlayerActionManager.PopMode(ActionMode.Flying)` — el jugador empieza a caer aunque siga con intención de volar, y para seguir volando tiene que volver a entrar (doble salto) — de ahí el "se quita y se pone" en bucle que describe Raúl. Esto explica también por qué los compañeros de party parpadean igual: `FollowPlayerState.OnUpdate()` decide cada frame si necesita seguimiento especial de vuelo leyendo `_playerActionManager.Top == ActionMode.Flying`, así que cada ciclo de salida/reentrada de vuelo del jugador dispara `ExitSpecialFollow()`/`OnSpecialModeEnter()` en cadena sobre ellos también, en sincronía exacta con el parpadeo del propio jugador. **Arreglado:** nuevo campo `_groundedWhileFlyingTimer` y constante `ExitFlightGroundedGrace = 0.12f` en `PlayerFlyingController` — `IsGrounded()` debe dar `true` de forma sostenida durante ese margen (no solo un frame) antes de aceptarlo como aterrizaje real y llamar a `ExitFlight()`; un roce de menos de 0.12s se ignora y el vuelo continúa con normalidad. Mismo patrón de histéresis que ya usa el proyecto para filtrar detecciones de un solo frame (`FollowPlayerState._isFollowingSticky`, INC-097). No se ha tocado `CanEnterFlight()` (usa su propio `IsGrounded()` para decidir si se puede ENTRAR en vuelo, un caso distinto sin el mismo problema de falsos positivos en bucle). **Pendiente de confirmar por Raúl en juego:** volar a baja altura/rozando el suelo y en picados cortos sin que la animación se corte sola, con y sin compañeros de party volando detrás.
...
### Sesión Septiembre 2026 — arreglos aplicados

- **INC-199 (15 sept 2026) — el minimapa y la UI in-world se ocultan solos al arrancar una misión nueva — `QuestMenuManager.cs` / `QuestLogListUI.cs`.** Reportado por Raúl: "cuando sale una misión nueva se esconde el minimapa y cosas de la UI". **Causa raíz:** al arrancar cualquier quest (`QuestManager.OnQuestStarted`), dos sitios muestran automáticamente el panel rápido de misiones (`QuestLogListUI`, el "quickMenu" de `QuestMenuManager`) como aviso tipo toast — `QuestLogListUI.OnQuestStarted()` lo muestra al instante, y por separado `QuestMenuManager.HandleQuestStarted()`/`AutoShowQuickMenu()` (flag `autoShowQuickOnQuestInit`) lo vuelve a mostrar tras `autoShowDelay` (0.35s); redundancia ya existente, no tocada en este fix. El problema real es que `QuestMenuManager.RefreshMenuRegistration()` (llamado cada `Update()`) trataba ese aviso automático exactamente igual que si el jugador hubiera abierto el menú de misiones a mano: en cuanto `quickMenu.IsVisible` es `true`, registra `MenuManager.RegisterOpen(MenuKind.Mission)`, lo que dispara `MenuManager.MenuOpened` — y a ese evento están suscritos, además del propio minimapa (`MinimapController.OnMenuOpened`, que oculta con cualquier `MenuKind` salvo `BigMap`), `SpeechBubbleUI`, `BossHealthBar`, `LorePopupUI`, `AbilityUnlockPopupUI` y `TutorialPromptUI` — todos se ocultaban en cuanto arrancaba cualquier misión, aunque el jugador no hubiera abierto ningún menú. **Arreglado:** `QuestLogListUI.ShowPanel()` gana un parámetro `isAutoToast` (por defecto `false`) y una propiedad pública `IsAutoToast` que queda `true` mientras el panel está visible solo por el aviso automático (se limpia al ocultarse, tanto por `ShowPanel(false, ...)` como por el auto-hide por tiempo, `AutoHideAfterDelay()`). Los dos puntos que muestran el aviso al arrancar una quest (`QuestLogListUI.OnQuestStarted()` y `QuestMenuManager.AutoShowQuickMenu()`) pasan `isAutoToast: true`; el resto de llamadas a `ShowPanel(true, ...)` (D-pad arriba, volver del menú principal) no pasan el parámetro y siguen registrando un menú real como hasta ahora. `QuestMenuManager.RefreshMenuRegistration()` ahora calcula `anyOpen` ignorando el quickMenu mientras `IsAutoToast` es `true`, así que el aviso de "nueva misión" se sigue viendo igual (mismo fade, mismo auto-hide a los `hideAfterSeconds`) pero ya no cuenta como menú abierto: el minimapa y el resto de UI in-world permanecen visibles. Abrir el menú de misiones a mano (D-pad arriba) sigue ocultándolos como siempre. No se ha tocado la redundancia de los dos triggers de auto-show (`QuestLogListUI.OnQuestStarted` + `QuestMenuManager.HandleQuestStarted`) por quedar fuera del alcance de este fix. **Pendiente de confirmar por Raúl en juego:** al recibir una misión nueva, el aviso de misiones aparece igual que antes pero el minimapa (y el resto de HUD in-world) no parpadea/desaparece; abrir el menú de misiones a mano con D-pad arriba sigue ocultando el minimapa con normalidad.

- **INC-200 (15 sept 2026) — aviso centrado de "misión nueva" — `QuestStartedBannerUI.cs` (nuevo).** Petición de Raúl: el único aviso de misión nueva era el panel rápido lateral (INC-199), discreto y fácil de no ver — "que salga algo centrado chulo". **Implementado:** nuevo componente `Assets/Scripts/UI/QuestStartedBannerUI.cs`, mismo patrón que `AbilityUnlockPopupUI` (`CanvasGroup` + DOTween, auto-dismiss por corrutina, se oculta si hay un menú/cinemática real encima vía `MenuManager.MenuOpened`/`MenuClosed`, implementa `ISceneBoundUIHideGuard` para no desaparecer a medio-mostrar en un cambio de escena) pero con animación de entrada tipo rebote (`DOAnchorPos(...).SetEase(Ease.OutBack)` cayendo desde `dropOffsetY` px por encima de su posición final) más un punch de escala en el icono (`DOPunchScale`), en vez del slide lateral del popup de habilidades. Se suscribe a `QuestManager.OnQuestStarted` con el mismo patrón robusto de espera que `QuestLogListUI.BindWhenReady()` (corrutina `while (QuestManager.Instance == null) yield return null`, re-suscripción si la instancia cambia). Texto: `string.Format(LocalizationManager.Instance.Get("QUEST_STARTED_BANNER", "Nueva misión: {0}"), questName)` — clave nueva añadida a `other_es.json`/`other_en.json` ("Nueva misión: {0}" / "New quest: {0}"). Para resolver `questName` sin `GetAll()`+LINQ en un evento (aunque no es un hot path, se mantiene la consistencia con el resto del proyecto) se añadió `QuestManager.TryGetQuestData(string questId, out QuestData data)`. **Deliberadamente FUERA del sistema de registro de menús:** a diferencia de `QuestLogListUI`/`QuestMenuManager`, este componente nunca llama a `MenuManager.RegisterOpen(...)` — no es un menú, así que no dispara `MenuManager.MenuOpened` y no oculta el minimapa ni el resto de HUD (justo lo que INC-199 corrigió para el aviso lateral); puede convivir con ese aviso lateral en pantalla a la vez sin parpadear entre los dos, ya que ninguno de los dos apaga al otro.

  **Pendiente de montaje en el Editor (no se ha podido hacer desde esta sesión — sin conexión al Editor de Unity, solo acceso a archivos):** el script está listo y compila de forma independiente, pero crear a mano el GameObject correspondiente dentro de `Start.unity` (la escena persistente donde ya vive `AbilityUnlockPopupUI`) editando el YAML de la escena a ciegas es demasiado arriesgado sin poder verificarlo visualmente — un error ahí afecta a la escena persistente que carga todo el juego. Pasos para Raúl en el Editor (2-3 minutos):
  1. En `Start.unity`, localizar el GameObject de `AbilityUnlockPopupUI` (mismo Canvas persistente) y duplicarlo (`Ctrl+D`) como punto de partida — ya trae `RectTransform` + `CanvasGroup` + `TextMeshProUGUI` + `Image` correctamente configurados.
  2. Renombrar la copia a `QuestStartedBanner`, quitarle el componente `AbilityUnlockPopupUI` y añadirle `QuestStartedBannerUI`.
  3. Reanclar el `RectTransform` raíz al borde superior-centro (anchors `(0.5, 1)`/`(0.5, 1)`, pivot `(0.5, 1)`) en vez de a la derecha, y ajustar el offset Y para que quede debajo de la barra de estado/reloj si la hay.
  4. Asignar en el Inspector: `Banner Root` (el propio `RectTransform`), `Banner Canvas Group`, `Title Text` (el `TextMeshProUGUI`), y opcionalmente `Icon` (dejar sin asignar si no se quiere icono todavía).
  5. Para el fondo, reutilizar uno de los sprites "cristal" ya usados en `Art/UI/Misiones/` (p.ej. `header_pill_bg.png` o `panel_glass_bg_compact.png`) en vez de traer un asset nuevo — mantiene la identidad visual ya unificada en esa UI.
  6. Sonido (opcional, decisión pendiente de Raúl): añadir una entrada nueva `UI_QuestStarted` en `Assets/_AUDIOPROFILE/AudioGraphProfile.asset` con el clip elegido, y poner `sfxEventKey = "UI_QuestStarted"` en el componente. Sin esta entrada el banner funciona igual, simplemente sin sonido (`AudioService.PlaySFX` no falla si la clave no existe, solo avisa en el Editor/dev build).
  7. Probar en Play arrancando cualquier quest (p.ej. hablar con Eldran) y confirmar que el banner entra con el rebote, se lee bien, y desaparece solo sin dejar el minimapa/HUD ocultos.
...
**Nota de corrección — 12 de septiembre de 2026:** los dos nodos que este análisis de julio daba por no construidos ya existen. `BranchOnQuestStateNode` se construyó como `BranchQuestStateNode` y el "esperar la próxima interacción con reentrada" se construyó como `WaitNpcInteractionNode` (ambos, 11 de septiembre de 2026) — confirmado leyendo el código, no por lo que dice esta sección. `WaitNpcInteractionNode` recibió además soporte para mostrar el icono de quest (cabeza + minimapa) el 12 de septiembre, vía `NarrativeActor.ShowQuestIcon`/`HideQuestIcon` (INC-196). El diagnóstico de fondo de este apartado — que el catálogo de julio no permitía expresar "diálogo/estado re-evaluado en cada interacción" — ya no aplica: no se leáis como plan pendiente. Detalle actualizado en `claude/catalogo-sistemas-legacy-vs-grafo-nuevo-2026-09-12.md`.
...
### Actualización (24 ago 2026) — causa real de "no aparecen nubes sueltas" encontrada y arreglada (sesión Cowork, sin Editor)

Raúl reportó dos síntomas juntos: (1) nunca ve ni una nube ambiental cruzando el cielo pese al cableado del 16 ago, y (2) el techo de nubes de tormenta se ve "demasiado bajo" cuando llueve. Investigado leyendo directamente los `.unity`/`.prefab` (serialización en texto, `EditorSettings.asset` confirma `m_SerializationMode: 2`), sin poder abrir el Editor. Una captura de Raúl (Scene view con 2 nubes sueltas visibles sobre las murallas, Game view igual de encuadre pero sin ninguna) fue la pista decisiva.

**Causa #1 — nubes sueltas invisibles, confirmada y arreglada:** los prefabs `QuibliRainCloud3D_1..4` (usados por `ambientCloudPrefabs`) viven en el layer 18, `Weather` (custom, ver `ProjectSettings/TagManager.asset`) — no en `Default` como el techo de tormenta de `CloudCoverSpawner` (`Cloud3D-MeshCarrier_*`/`Cloud3D-Sphere`, layer 0). La cámara de juego real (no las ~30 `Camera` sueltas de `MainWorld.unity`, todas deshabilitadas o con GameObject inactivo, ni la de `Start.unity`, bajo un `START_Dedication` inactivo — la de verdad vive dentro de `Assets/Prefabs/_WILL.prefab`, la única de los tres personajes jugables con cámara propia) tenía `m_CullingMask.m_Bits: 65535` — exactamente los 16 layers 0-15, **sin incluir Minimap(16)/AmbientZone(17, ambos a propósito, tienen su propio uso) ni Weather(18)**. Como la nube ambiental nunca estuvo en un layer que la cámara principal renderizara, el sistema entero (spawn, movimiento, fundido) funcionaba perfectamente pero nunca se veía nada en Game view — solo en Scene view del Editor, que ignora el culling mask de cualquier cámara. **Fix aplicado:** `m_Bits` de esa cámara (fileID `1062031804521613182` en `_WILL.prefab`) → `327679` (añade el bit de `Weather`, sin tocar Minimap/AmbientZone). De paso, se confirmó que la duda pendiente del 16 ago sobre `shaderMode` de `AmbientCloudDrifter` (asumía `QuibliCloud3D`) es correcta: el material real de `QuibliRainCloud3D_1.prefab` es `SampleScene_Cloud3D.mat`, que usa el shader Quibli/Cloud3D — sin bug ahí, punto pendiente ya cerrado.

**Causa #2 — techo de tormenta "demasiado bajo", diagnóstico razonado y arreglado (no confirmado visualmente todavía):** `CloudCoverSpawner.CheckRecenter()` recentraba el techo en X/Z cuando el jugador se aleja, pero conservaba la altura Y tal cual se calculó la PRIMERA vez que se formó el techo (`newPos.y = rootPos.y`) — pensado solo para no perder `_safetyHeightBonus`. Si el jugador sube de cota real después de que empezó a llover (una cuesta, una montaña — el mundo tiene desnivel visible, ver captura de Raúl con picos nevados de fondo), el techo se queda anclado a la altura absoluta de donde empezó la tormenta: cuanto más suba el jugador, más bajas se ven las nubes relativas a él, sin que el sistema de `horizonElevationDegrees` (fix del 20 ago) pueda compensarlo porque ese fix solo resuelve el ángulo de elevación EN HORIZONTAL, no un desnivel vertical del terreno. **Fix aplicado:** `CheckRecenter()` ahora recalcula la altura igual que `BuildCoverIfNeeded()` (`cloudHeight` sobre la posición Y ACTUAL del jugador, no la de cuando empezó a llover), conservando `_safetyHeightBonus`.

**Pendiente (requiere Unity Editor, fuera del alcance de esta sesión):**
1. Confirmar en el juego que las nubes sueltas ya se ven en Game view tras el fix del culling mask.
2. Playtest de una tormenta subiendo de cota real (p. ej. hacia la zona de montaña) para confirmar que el techo ya no se queda bajo — si sigue viéndose bajo cerca del centro (radio≈0 del punto donde empezó a llover), es el punto ciego ya documentado de `horizonElevationDegrees` (inevitable con cualquier altura, ver tooltip), no un bug nuevo.
3. Si `Sendero.unity`/`CandyLand.unity`/`PlayerTest` tienen su propia copia de `_WILL` con una cámara distinta (no debería, `_WILL.prefab` es el mismo asset en las cuatro escenas), confirmar que el fix se refleja también ahí sin overrides locales de `m_CullingMask` (no se encontró ninguno en `MainWorld.unity` al buscar el fileID de la cámara).

### Actualización (25 ago 2026) — INC-100, "las nubes salen en interiores" (nubes sueltas ambientales)

Raúl reportó: "las nubes estan saliendo en interiores". Investigado leyendo código, sin Editor.

**Causa:** `AmbientCloudDirector` (las nubes sueltas de buen tiempo, activado el 16 ago — ver arriba) nunca se suscribió a `EnvironmentController.OnInteriorEntered`/`OnInteriorExited`. Los otros cuatro sistemas de cielo/clima de la misma familia sí lo hacen: `CloudCoverSpawner` (el techo de tormenta, arreglado el 16 ago — este mismo bug, "las nubes se ven en un interior", ver más arriba), `DayNightCycle` (lluvia/niebla), `NightSkyStarSpawner` y `NightSkyConstellationSpawner`. `AmbientCloudDirector` se creó el 15 ago (un día antes de que ese patrón de suscripción se estableciera en `CloudCoverSpawner`) y nadie volvió a revisarlo contra los sistemas hermanos después — se quedó generando y mostrando nubes sueltas sin importar si el jugador está dentro de un interior (`EnvironmentController.IsEffectivelyInterior`), así que cualquier interior sin techo suficientemente alto/opaco para taparlas (o simplemente porque el `adjustCameraClipping`/`interiorFarClipPlane` de su `AnchorEnvironment` no recorta lo bastante) las deja visibles por encima.

**Fix aplicado en `Assets/Scripts/World/AmbientCloudDirector.cs`:**
- Suscripción a `EnvironmentController.OnInteriorEntered`/`OnInteriorExited` en `OnEnable`/`OnDisable`, mismo patrón que `CloudCoverSpawner`, incluyendo el estado inicial (`EnvironmentController.Instance.IsEffectivelyInterior`) por si la escena se carga ya dentro de un interior.
- `CloudSpawnLoop()` deja de generar nubes nuevas mientras `_hiddenByInterior` es true (igual que ya hacía con `_stormActive`).
- Las nubes ya en vuelo no se cortan (mismo criterio que con una tormenta): se ocultan vía `SetRenderersVisible`, no `SetActive(false)` — `SetActive` pararía su corrutina de vuelo sin reanudarla sola (ver doc de `AmbientCloudDrifter.SetRenderersVisible`).
- **Colisión evitada con el enfoque de cámara de diálogos:** `SetAmbientCloudsVisible(bool)` (llamado desde `DialogueCinematicController` para ocultar nubes sueltas un momento durante una cinemática) y el nuevo estado de interior competían por la misma visibilidad — sin cuidado, una cinemática que termina (`SetAmbientCloudsVisible(true)`) mientras el jugador sigue dentro de un interior habría revelado las nubes igualmente. Resuelto con dos flags independientes (`_visibleRequested`, la última petición externa; `_hiddenByInterior`, el estado real) combinados en `ApplyCloudVisibility()` (visible = ambas cosas a la vez), en vez de que cualquiera de las dos pisara a la otra directamente.

**Pendiente (requiere Unity Editor):** confirmar en el juego que ya no se ven nubes sueltas por encima de ningún interior (casa, taberna, mazmorra), y que una cinemática de diálogo dentro de un interior sigue sin mostrarlas al terminar. Añadido a `TRACKER.md` como INC-100.

### Actualización (25 ago 2026, 2ª pasada) — INC-100 KO, causa real: flujo cinemático nunca dispara los eventos de interior

Raúl confirmó KO en juego, con capturas: dentro de la cinemática de la Taberna (minijuego de comida, `TabernaSequencer`) las nubes sueltas seguían viéndose por encima de la escena. Pista suya: "lo que determina si estamos en interiores es el environment Anchor".

**Causa real (el fix de la 1ª pasada era insuficiente, no incorrecto):** `EnvironmentController.OnInteriorEntered`/`OnInteriorExited` (a los que `AmbientCloudDirector` se suscribió en la 1ª pasada) **solo se disparan desde el flujo "andando"** (`ApplyInterior`/`ApplyExterior`, el que usa `TeleportService`/`AnchorSetter` al cruzar un `AnchorEnvironment` caminando). El flujo cinemático (`CinematicSequencerBase`/`SimpleCinematicDirector` → `BeginCinematicOverride` + `ApplyInteriorForCinematic`, que es como `TabernaSequencer` entra en su interior) **nunca pasa por ahí a propósito** — está documentado en el propio comentario de `DayNightCycle`, que tuvo este MISMO bug con la lluvia/niebla y lo arregló sondeando `EnvironmentController.IsEffectivelyInterior` (que sí tiene en cuenta el override cinemático) en su `Update()`, con un patrón edge-triggered, en vez de depender solo del evento.

`AmbientCloudDirector` nunca copió ese sondeo — solo el evento —, así que quedaba ciego exactamente al caso que reportó Raúl.

**Fix aplicado (2ª pasada) en `Assets/Scripts/World/AmbientCloudDirector.cs`:** `Update()` nuevo que sondea `EnvironmentController.Instance.IsEffectivelyInterior` cada frame, edge-triggered (dispara `HandleInteriorEntered()`/`HandleInteriorExited()` solo al cambiar), exactamente igual que `DayNightCycle.Update()`. El evento de la 1ª pasada se deja tal cual (reacción inmediata para el flujo andando, sin esperar al siguiente frame); el sondeo es la red de seguridad que además cubre el cinemático.

**Mismo hueco corregido de forma preventiva** en los otros tres sistemas de la misma familia que tampoco sondeaban `IsEffectivelyInterior` (solo el evento, igual que `AmbientCloudDirector` en la 1ª pasada) — no confirmados KO en juego para estos tres, pero comparten la misma causa raíz:
- `CloudCoverSpawner` (techo de nubes de tormenta).
- `NightSkyStarSpawner` (polvo de estrellas de fondo).
- `NightSkyConstellationSpawner` (constelaciones, componente nuevo de hoy — ver más abajo).

**Pendiente (requiere Unity Editor):** confirmar en juego que las nubes sueltas ya no se ven dentro de la cinemática de la Taberna, y de paso que el techo de tormenta/estrellas/constelaciones tampoco se cuelan en ningún interior cinemático si llegara a coincidir (lluvia + cinemática de interior, o de noche + cinemática de interior).

### Nota (25 ago 2026) — INC-101, constelaciones: "se borran algunas estrellas guía mirando de frente, reaparecen girando la cámara" — diagnosticado y arreglado

Junto con el KO de INC-100, Raúl reportó algo nuevo en `NightSkyConstellationSpawner` (componente nuevo de hoy, ver `claude/constelaciones-nocturnas-2026-08-25.md`): mirando de frente a una constelación esta "se borra", y girando un poco la cámara hacia un lado se ve completa. Preguntado qué parte exacta desaparecía (para no arriesgar un fix a ciegas): **respuesta de Raúl — "solo algunas estrellas/puntas"** (las líneas se seguían viendo bien).

**Causa encontrada leyendo código:** cada estrella guía se coloca en `SpawnBeaconStar` en `direction * _activeRadius` (línea ~371); cada extremo de línea se coloca en `SpawnLine` en la misma expresión exacta, `directions[polyline[i]] * _activeRadius` (línea ~400) — dos GameObjects distintos ocupando el MISMO punto 3D exacto donde una línea conecta con una estrella. Tanto `_starMaterial` como `_lineMaterial` son `Sprites/Default` (transparente, alpha-blended, sin `ZWrite`), así que Unity desempata el orden de dibujo entre transparentes por distancia a cámara cuando están en la misma `sortingLayer`/`sortingOrder` (ninguno de los dos lo tenía asignado explícitamente, ambos a 0 por defecto). En un punto EXACTAMENTE compartido esa distancia es prácticamente idéntica para los dos renderers, así que el orden de dibujo queda inestable — depende de redondeos de coma flotante que cambian con el ángulo de vista. Justo al mirar derecho al vértice (máxima coincidencia geométrica desde el punto de vista de la cámara) es cuando más fácil resulta que la línea (más tenue, `lineBrightness=0.5`) gane el desempate y tape a la estrella; girar la cámara rompe el empate numérico y la estrella reaparece — encaja exactamente con el síntoma descrito.

**Fix aplicado en `Assets/Scripts/World/NightSkyConstellationSpawner.cs`:** `sortingOrder` explícito — `1` para las estrellas guía (`SpawnBeaconStar`), `0` para las líneas (`SpawnLine`) — así las estrellas SIEMPRE dibujan después de (encima de) las líneas, sin depender de ningún desempate por distancia. De paso refuerza la intención de diseño ya declarada en el comentario de clase del script ("las líneas son solo una guía discreta, el ojo se va primero a las estrellas").

**Pendiente (requiere Unity Editor):** confirmar en juego que ya no se borra ninguna estrella guía mirando de frente a ninguna de las tres constelaciones (Espada/Corona/EstrellaGuia).
...
> **ACTUALIZACIÓN (1 sept 2026):** resto de D.2 implementado — Will, Liam (con la discusión
> reactiva con Estela) y, además de los tres nombres originales del diseño, Eldran (también puede
> unirse al grupo, así que se le da el mismo tratamiento). Tres componentes nuevos, misma familia
> que `EstelaIdleCommentary` y mismo mecanismo de `CinematicSequence` descrito arriba:
> - `WillIdleCommentary.cs`: comentario suelto + numerito de "buscar algo" (se para, gesto
>   `Question01`, "¿dónde habré puesto esto...?").
> - `LiamIdleCommentary.cs`: comentario suelto propio + pieza REACTIVA — se suscribe a un evento
>   estático nuevo de `EstelaIdleCommentary` (`EstelaSatDownHungry`, disparado cuando ella se sienta
>   a quejarse de hambre) y, con una probabilidad y un cooldown configurables, se gira hacia ella,
>   le dice que se levante, ella replica, y Liam corta su numerito antes de tiempo llamando a
>   `EstelaIdleCommentary.RequestStandUp()` — la "discusión entre los dos" que pedía el diseño
>   original. Requirió añadir a `EstelaIdleCommentary` un método `RequestStandUp()` público y que
>   `EstelaSitAction` exponga un `ForceComplete()` interno (su `IsCompleted` es `protected set`, no
>   se puede tocar desde fuera de la jerarquía de `CinematicAction`).
> - `EldranIdleCommentary.cs`: comentario suelto de mentor protector + numerito de "se le olvida el
>   báculo" (se para, gesto `Question02`, "un momento... ¿dónde he dejado el báculo?" / "ah, no,
>   aquí está").
>
> Los cuatro componentes (Estela incluida) añadidos a sus respectivos GameObjects vía edición
> directa del YAML del prefab (sin pasar por el Editor de Unity): `_ESTELA.prefab`, `_WILL_NPC.prefab`
> (no `_WILL.prefab` — ese es el rig del controlador de Invector para cuando el jugador toma el
> control de Will, sin ninguno de los componentes de NPC; el que sí tiene `NPCBehaviourManagerV2`
> es `_WILL_NPC.prefab`), `_LIAM.prefab` y `Assets/_NPCs/Eldran.prefab`.
>
> **Bug real encontrado y corregido en los cuatro componentes (Estela incluida) durante esta
> pasada:** el cacheo de `NPCPartyMember`/`NPCBehaviourManagerV2` vía `GetComponent<>()` estaba en
> `Awake()`, pero `NPCPartyMember` se añade en tiempo de ejecución dentro del propio `Awake()` de
> `NPCBehaviourManagerV2` (`EnsureRequiredComponents()`) — Unity no garantiza qué `Awake()` de los
> dos corre antes en el mismo GameObject sin `[DefaultExecutionOrder]`. Si el de estos cuatro
> componentes corría antes, `_partyMember` se quedaba en `null` para siempre y el numerito no
> disparaba nunca en ese personaje, en silencio (solo un log de diagnóstico bajo
> `UNITY_EDITOR`/`DEVELOPMENT_BUILD`, fácil de pasar por alto). Movido el cacheo de `Awake()` a
> `Start()` (que Unity sí garantiza que corre después de todos los `Awake()` de la escena) en los
> cuatro archivos.
>
> Pendiente real: confirmar en el Editor/en juego que los cuatro disparan con normalidad (ver fila
> de `TRACKER.md` de esta misma fecha para el checklist completo), y decidir si Eldran encaja mejor
> con un segundo numerito de mentor distinto más adelante (de momento comparte estructura con
> Will/Estela: comentario suelto + una pieza física).

> **CORRECCIÓN CRÍTICA (1 sept 2026, misma fecha) — INC-151, regresión real encontrada al probar lo
> de arriba:** los cuatro componentes llevaban `[RequireComponent(typeof(NPCPartyMember))]`. Unity
> aplica `RequireComponent` al construir el GameObject, antes de que corra ningún `Awake()` —  como
> `NPCPartyMember` no está serializado en ningún prefab (se añade a mano en runtime, ver arriba),
> Unity lo auto-instanciaba en blanco en ese momento. `NPCBehaviourManagerV2.EnsureRequiredComponents()`
> (Módulo 4, Companion) entonces encontraba `GetComponent<NPCPartyMember>()` ya no-null y se saltaba
> `partyMember.SetConfig(configuration.partyConfig)` — el NPC quedaba en el party con
> `NPCPartyMember` pero sin `PartyConfig`, justo el síntoma sin explicar de `INC-149` (ahí solo
> afectaba a Estela; al wirear Liam con el mismo atributo se rompió también, y ahí se acabó de
> confirmar la causa). **Fix: quitado el atributo de los cuatro archivos** — el `GetComponent<NPCPartyMember>()`
> en `Start()` ya maneja el caso `null` con seguridad, no hacía falta el atributo para nada más que
> documentar la dependencia, y era él mismo la causa del bug. Ninguno de los cuatro prefabs tenía el
> componente grabado en disco (solo vivía en memoria durante Play Mode), así que no hizo falta tocar
> YAML de nuevo. Detalle completo: `TRACKER.md` INC-151.
...
- **`TDD.md` (este documento) es la fuente de verdad única para documentación técnica**: arquitectura, sistemas, reglas de código, bugs conocidos, troubleshooting, diseños en curso, checklists de proceso y auditorías.
- **`GDD.md` (desde el 24 ago 2026) es la fuente de verdad única para documentación de diseño y narrativa**: historia, guión técnico, fichas de personaje, balance de hechizos/contenido, estado de diálogos. Antes de esta fecha ese contenido vivía en un Google Doc separado ("GDD" v1.0) que se quedó desactualizado sin que nadie lo notara (ejemplo real: la ficha de Liam contradecía una corrección de lore ya confirmada) — se migró a `GDD.md` precisamente para que le aplique la misma disciplina anti-fragmentación que a este documento. Es la segunda y última excepción de tipo "documento raíz", no un precedente para crear más.
- **Cualquier `.md` nuevo que documente algo de sustancia** (una auditoría, un análisis de diseño técnico, una decisión de arquitectura, un checklist) **se añade como sección nueva de `TDD.md`** si es contenido técnico, o **de `GDD.md`** si es contenido de diseño/narrativa — nunca como archivo suelto nuevo en la raíz. Al añadirlo: crear la sección al final (o dentro de la sección temática que corresponda si es una ampliación de algo ya documentado), actualizar el índice, y evitar repetir contenido que ya exista en otra sección — enlazar a ella en su lugar de copiarlo.
- **Excepciones — se quedan como archivos aparte, pero cortos y sin contenido duplicado:**
  ...
  - **`AGENTS.md` / `CLAUDE.md`** — herramientas de IA (Claude Code, Cursor, etc.) los leen **automáticamente** como contexto de proyecto en cada sesión; no son solo documentación para humanos y por eso no se retiran ni se fusionan aquí dentro. Se mantienen como resumen corto de las reglas no negociables con pointers a `TDD.md` y `GDD.md` para el detalle — nunca como copia completa de ninguna sección. Si `TDD.md` cambia una regla no negociable o un invariante narrativo, o `GDD.md` cambia algo estructural (personajes, sistemas de balance), hay que reflejar el resumen en estos dos archivos también (se mantienen sincronizados, no es "documentación muerta").
  - **`GDD.md`** — ver arriba: mismo criterio que este documento, pero para diseño/narrativa en vez de técnica.
  - **`TRACKER.md`** (desde el 24 ago 2026) — fuente de verdad única para el seguimiento de incidencias (bugs, mejoras, funciones): ID, tipo, categoría, prioridad, estado y validación. Sustituye al Tracker en Google Sheets (que se mantenía a mano y no podía escribirse directamente desde una sesión de IA) y absorbe la numeración `INC-XXX` que hasta hoy llevaba también, por su cuenta y sin saberlo, el catálogo de bugs de § 13 — ver la nota al principio de § 13 y la tabla de correspondencia en `TRACKER.md`. El detalle técnico largo de cada bug (causa raíz, código) se queda en § 13 o en la sección temática que corresponda; `TRACKER.md` solo lleva la fila corta de seguimiento. Es la tercera y última excepción de tipo "documento raíz", no un precedente para crear más.
- Cualquier otro `.md` de sustancia que hoy exista o se cree en el futuro (una auditoría, un checklist, un análisis técnico) va como sección de `TDD.md`; cualquier contenido de diseño/narrativa va como sección de `GDD.md`; cualquier incidencia nueva (bug/mejora/función) va como fila nueva de `TRACKER.md`, con su detalle técnico (si lo necesita) como sección de `TDD.md`. Ninguno de los tres se fragmenta en archivos nuevos en la raíz.
...
### 21.10 Segundo intento (4 sep 2026) — hierba junto a los árboles, esta vez como herramienta de Editor

Raúl pidió retomar el vestido de hierba de §21.6, explicando el motivo real de por qué la quitó en su momento (§21.9 solo documentaba la caza de un bug de textura que se descartó como causa): **la hierba se metía dentro de los árboles**. Tenía sentido con cómo se generó la primera vez — jitter ciego de ±2.5 unidades alrededor de cada árbol, sin comprobar el hueco real que ocupa cada tronco/copa, y encima editando el `.unity` directamente como texto sin poder ver el resultado en el Editor.

**Este intento cambia de método, no solo de números.** En vez de volver a editar el archivo de escena a ciegas, se ha escrito `Assets/Scripts/Editor/QuibliGrassDresser.cs` — una herramienta de menú (`El Sendero → Mundo → Vestir Árboles con Hierba (Quibli)`) que Raúl ejecuta dentro de Unity, mismo patrón que el resto de builders del proyecto (idempotente, deja ver el resultado en el Editor antes de guardar). Dos correcciones de fondo sobre el intento anterior:

1. **Radio de exclusión real en vez de jitter ciego:** cada árbol de `Fantasy_Kingdom_Pack` ya trae de fábrica un `NavMeshObstacle` (pensado para que los NPCs no lo atraviesen al caminar) con un tamaño que aproxima bien su tronco/copa a nivel de suelo — confirmado en `Tree02_d01.prefab` (Box, extents ~2.71×2.71). La herramienta lee ese componente por instancia y coloca la hierba en un anillo claramente fuera de él (radio de exclusión + margen), en vez de un jitter que podía caer a cualquier distancia del tronco, incluida la mitad del hueco donde ya vive el propio árbol.
2. **Collider de la prefab de hierba desactivado:** `Nature - Grass Patch Long/Short.prefab` (de la demo de Quibli) trae un `SphereCollider` sólido (`m_IsTrigger: 0`, radio 0.3) — razonable para una única pieza de muestra, pero con cientos de instancias repartidas por el suelo bloquearía físicamente al jugador. La herramienta lo desactiva en cada instancia que crea.

También ajusta la altura al `Terrain.activeTerrain` real (`SampleHeight`) en vez de copiar la altura del propio árbol, y agrupa todo bajo el mismo nombre de raíz que usaba §21.6 (`Quibli - Hierba junto a árboles`) para mantener continuidad de nombres si se busca en el histórico.

**Deliberadamente NO incluye los rayos de sol de §21.6 en esta pasada.** `SunRays.prefab` es un quad plano (no volumétrico) — un elemento mucho más dependiente del ángulo de cámara concreto desde el que se mire, que muy probablemente se vea raro desde según qué ángulos sin poder verlo renderizado antes. Se deja pendiente como paso aparte si Raúl quiere retomarlo después de valorar cómo queda la hierba.

**Pendiente de que Raúl ejecute el menú en el Editor, mire el resultado y confirme si el anillo de exclusión es suficiente** (los valores de margen/ancho de anillo están como constantes al principio del script, fáciles de ajustar si aún se ve demasiado pegado o demasiado disperso).

### 21.11 Actualización (4 sep 2026, misma sesión) — más densidad, relleno general del campo

Raúl probó el menú de §21.10 en el Editor (funcionó sin errores, capturas de pantalla confirmando que la hierba ya no se mete en los troncos) pero pidió mucha más cantidad — que el campo se vea relleno de hierba, no solo unas matas sueltas junto a cada árbol.

`QuibliGrassDresser.cs` ahora hace dos pasadas en vez de una: el anillo junto a cada tronco (subido de 2 a 4 parches por árbol) más una **rejilla de relleno general** sobre toda el área donde hay árboles (con jitter para no verse en cuadrícula perfecta, separación ~3.2 unidades, tope de seguridad de 3.500 instancias para no disparar el tamaño de la escena). El filtro de "dónde puede caer hierba de relleno" no usa las capas de textura del terreno (habría que identificar a ciegas cuál de la decena de `.terrainlayer` de orígenes distintos es "hierba" — ver el análisis de §19 sobre mezcla de packs) sino la cercanía al árbol más próximo: si un punto está a menos de ~20 unidades de algún árbol, se asume pradera/bosque (ahí es donde se plantaron árboles); si está más lejos, se asume plaza del castillo, montaña o agua, y no se coloca nada.

Deliberadamente se descartó migrar esto al sistema de "Terrain Detail" de Unity (la forma correcta de pintar césped realmente denso a coste de rendimiento bajo) para no introducir una herramienta nueva sin poder verla renderizada en la misma sesión — sigue siendo la misma prefab de la demo de Quibli que Raúl ya vio funcionar, solo que con muchas más instancias. Si el resultado sigue sin convencer en densidad, la separación de la rejilla y el tope de instancias están como constantes al principio del script.

### 21.12 Arbustos junto a los árboles (4 sep 2026, misma sesión)

Raúl notó que Quibli también trae arbustos en su demo de naturaleza y pidió añadir "algunos" — a diferencia de la hierba (pensada para rellenar todo el campo), aquí se buscaba un toque disperso, no una alfombra.

Nuevo script `Assets/Scripts/Editor/QuibliBushDresser.cs`, menú **El Sendero → Mundo → Añadir Arbustos junto a Árboles (Quibli)**. En vez de instanciar una prefab, construye el GameObject a mano (`MeshFilter` + `MeshRenderer`) usando directamente las mallas ya exportadas por la herramienta "Foliage Generator" de Quibli en `Assets/Plugins/Quibli/Demos/[Common]/Models/Foliage Generator Exported Meshes/` (9 variantes de silueta) junto con el material `NatureScene_BillboardBush_01 1.mat` — el mismo material base (sin recolorear) del que salió el rosal rosa de la secuencia final (§21.3); el original sin tocar es el arbusto verde normal del pack, así que es seguro para vestido general.

Reutiliza la misma técnica de exclusión por `NavMeshObstacle` del árbol que `QuibliGrassDresser.cs` para no invadir el tronco. Probabilidad inicial: 35% de los árboles con un arbusto, 12% de esos con un segundo al otro lado. Tras probarlo Raúl pidió "alguno más" (no una alfombra, solo un poco más denso) y se subió a 50% / 18% — el script es idempotente así que solo hizo falta volver a ejecutarlo con las constantes actualizadas. Confirmado por Raúl: "lo veo bien".

### 21.13 Rayos de sol junto a los árboles (4 sep 2026, misma sesión)

Último punto pendiente del vestido de MainWorld (§21.6 lo dejó fuera junto con la hierba, ver §21.9). Requisito explícito de Raúl al retomarlo: "si conseguimos que se vean lo deben hacer solo si es de dia y no esta nublado o llueve".

Tres archivos nuevos:

- `Assets/Scripts/Editor/QuibliSunRayDresser.cs` — menú **El Sendero → Mundo → Añadir Rayos de Sol junto a Árboles (Quibli)**. Coloca un rayo (dos planos Quad cruzados a 90°, con el material `NatureScene_LightBeam 1/2.mat` de la demo de naturaleza de Quibli) junto a una selección dispersa de árboles — ~9% de probabilidad por árbol, tope de seguridad de 18 en total, buscando el mismo acento puntual que la propia demo de Quibli usa (solo ~5 en toda su escena), no un relleno como la hierba. No se reutiliza la malla exacta de la demo (viene embebida en su escena, no es un asset aparte) sino un Quad simple, porque el shader `Quibli/Light Beam` ya resuelve el difuminado de borde por UV y por profundidad — la silueta del "rayo" la pone el shader, no la malla.
- `Assets/Scripts/World/SunRayBeam.cs` — componente `[ExecuteAlways]` en cada plano: reorienta el rayo en cada frame para que siga la dirección real de la luz direccional de la escena. Necesario porque `DayNightCycle` cambia el ángulo del sol entre Amanecer/Día/Atardecer (`timeSettings`) — un rayo con rotación fija solo se vería alineado con el sol en uno de esos tres periodos.
- `Assets/Scripts/World/SunRayWeatherGate.cs` — componente en el grupo raíz que oculta/muestra los rayos según `DayNightCycle`: visibles solo en Amanecer/Día/Atardecer (nunca de noche), nunca con `IsRaining`, y nunca durante el "cielo nublándose" previo a la lluvia (evento `CloudsBuildingUp`). Con un margen de seguridad de 12s para el caso raro en que esa nubosidad se cancele sin llegar a llover (`StopRain` a mitad de `CloudBuildUpRoutine` no dispara `RainStarted`/`RainStopped`, así que sin ese margen los rayos se podrían quedar ocultos indefinidamente).

De los tres efectos de vestido de esta sesión (hierba, arbustos, rayos), este es el más especulativo: no hay forma de previsualizarlo desde aquí, depende tanto del ángulo de cámara como del ángulo del sol en cada momento del ciclo. Pendiente de que Raúl lo pruebe en el Editor y ajuste cantidad/escala/altura si hace falta.

### 21.14 Corrección (4 sep 2026, misma sesión) — los rayos de §21.13 no se veían

Raúl probó el menú de §21.13 y reportó "no se ve" (capturas: el plano existe, seleccionado y bien situado junto al árbol en el Inspector/Hierarchy, pero no renderiza nada visible).

Diagnóstico: leyendo `LightBeam.shader` con calma, `fade_uv = smoothstep(1, 0, abs(uv*2-1)) ^ _UvFade` — un exponente bajo da un difuminado ancho/suave y uno alto da una franja muy estrecha. Los materiales de la demo (`NatureScene_LightBeam 1/2.mat`) tienen `_UvFadeX ≈ 0.4-0.65` (suave) y `_UvFadeY ≈ 4` (franja estrecha), pensados para su malla curva original donde el eje U recorre el LARGO del rayo y el V su ANCHO. En Unity, el eje U de un Quad corresponde al eje local X y el V al Y. El primer `QuibliSunRayDresser.cs` puso el LARGO del plano en Y en vez de en X — es decir, la franja estrecha (exponente ~4) caía sobre el largo entero del rayo en vez de sobre su ancho, dejando visible solo un hilo minúsculo casi transparente. No era un problema del pipeline de render (URP con `m_RequireDepthTexture: 1`, confirmado) ni de los materiales en sí (sus valores son coherentes entre variantes 1 y 2, así que no se tocaron).

Corregido en dos archivos: `QuibliSunRayDresser.cs` ahora escala `new Vector3(largo, ancho, 1f)` (antes al revés), y `SunRayBeam.cs` alinea el eje local X del plano contra el sol (`Quaternion.FromToRotation(Vector3.right, ...)`, antes `Vector3.up`) y gira `giroPropio` alrededor de X en vez de Y, para que el plano siga apuntando correctamente tras el cambio de ejes. Pendiente de que Raúl lo vuelva a probar.

### 21.15 INC-124, Fase 1 — primer lote de cohesión visual: Rim en FK01-04 (5 sep 2026)

A petición de Raúl ("entonces es cierto que apenas estamos usando el quibli no? pues del tiron vamos con eso"), arranca la Fase 1 de `INC-124` (ajuste material a material para acercar MainWorld al look de la demo "City" de Quibli — ver `claude/analisis-quibli-city-demo-vs-mainworld-2026-08-30.md`). Empezando, tal como proponía el plan, por `Fantasy_Kingdom_Pack` — en concreto los 4 materiales de paredes de edificio (`FK01`–`FK04`, cada uno un atlas con varias texturas de piedra/madera/puertas/ventanas).

**Copia de seguridad ANTES de tocar nada** (a petición explícita de Raúl: "memoriza bien cada cambio... por si hay que revertir"): los 4 `.mat` originales, tal cual estaban, están copiados en `_ClaudeBackups/CohesionTuner-2026-09-05-FK01-04/` (carpeta fuera de `Assets/`, para que Unity no la trate como assets duplicados). Para revertir este lote: copiar esos 4 archivos de vuelta encima de `Assets/Art/World/Fantasy_Kingdom_Pack/Materials/FK0{1,2,3,4}.mat` y dejar que Unity reimporte.

**Qué se cambió y por qué:** en los 4 materiales, `_RimEnabled` de `0` a `1`, más sus 4 parámetros asociados (`_FlatRimColor`, `_FlatRimLightAlign`, `_FlatRimSize`, `_FlatRimEdgeSmoothness`) — copiados TAL CUAL de `City_Building_Wall_3.mat` de la demo (un material real, ya probado, no valores inventados): `_FlatRimColor: {r: 0.39157256, g: 0.33716366, b: 0.2831488, a: 0.85056}` (un tono cálido marrón, no un brillo llamativo), `_FlatRimLightAlign: 0.234`, `_FlatRimSize: 0.617`, `_FlatRimEdgeSmoothness: 0.148`. También hubo que añadir manualmente `DR_RIM_ON` a `m_ValidKeywords` en el YAML — el toggle `[Toggle(DR_RIM_ON)]` del shader activa una keyword además del valor float, y al editar el `.mat` a mano (no desde el Inspector) hay que sincronizar las dos cosas o el toggle no tiene efecto visual aunque el float diga `1`.

**Qué NO se tocó, y por qué:** `_TextureImpact` y `_GradientEnabled` se dejaron intactos (en `1` y `0` respectivamente) en este primer lote. Muestreando 12 paredes + 2 pilares de la propia demo de Quibli, `_TextureImpact` varía muchísimo (de 0 a 1, sin patrón obvio por nombre) y casi la mitad de las paredes de la demo lo tienen en `0` — es decir, con la textura real completamente apagada, sustituida por color plano de shader. Bajar ese valor en un atlas como `FK01-04` (que mezcla piedra, madera, puertas con vidriera y varios colores de ladrillo en una sola textura) tiene pinta de reproducir el mismo fallo ya documentado en `INC-112` (Golem "blanco/lavado" al bajar `_TextureImpact`, revertido). El Rim, en cambio, es un aditivo sobre el color final — no sustituye la textura — así que es un cambio de mucho menor riesgo para probar primero.

**Pendiente:** que Raúl abra el Editor (Unity debería reimportar los 4 materiales solo) y mire cómo queda el Rim en los edificios del Reino antes de seguir con más materiales (Terrain/Ground/Farm/Flower del mismo pack, u otros packs). Si no convence, revertir con la copia de seguridad de arriba es inmediato.

### Maqueta alternativa Eldoria Codex (10 septiembre 2026 — INC-188)

> Las notas de las primeras pasadas que siguen son históricas. El estado más reciente está en «Revisión urbana tras la prueba de Raúl» al final de esta sección.

`Assets/Editor/EldoriaCodexBuilder.cs` ofrece `El Sendero/Eldoria Codex/Crear nueva maqueta del mapa`. Crea una carpeta `Assets/Scenes/Worlds/EldoriaCodex_yyyyMMdd_HHmmss` con una escena `Eldoria_Codex.unity` y sus recursos exclusivos. Abre las demos 09, 05, 06 y 10 como previews, copia su composición sin terrenos/cámaras/luces de demo y mide sus huellas para preparar plataformas; conserva la escala original. La escena de trabajo anterior se mantiene abierta y vuelve a ser activa. Cada ejecución crea una variante nueva.

Es una maqueta de composición pendiente de generar y revisar, no un mundo jugable terminado: no instala WorldBootstrap, anclas, NPCs narrativos ni NavMesh. Los desniveles internos de las demos requieren revisión al sustituir su terreno por plataformas; también las cascadas, el puerto y las intersecciones del río. La demo 08 queda como alternativa sin incorporar. La compilación aislada pasó con el compilador y referencias del proyecto; la ejecución batch fue rechazada por otra instancia de Unity abierta. No se ha efectuado inspección visual. La generación invoca la validación Interactive/Grafo y deja su resultado en Informe.txt; esa validación todavía no se ha ejecutado.

**Revisión tras primera ejecución de Raúl (10 septiembre 2026):** la escena sí se generó; sus capturas evidencian que importar la demo 10 completa aplanaba gran parte del macizo. Se sustituye por ocho casas BuildingAT35/BuildingAT53 a escala original alrededor de una plaza abierta al sur, con meseta elíptica y transición de 90 m. Se elimina cada objeto cámara junto con sus efectos legacy dependientes, conservando sus hijos; se corrige el winding de las cintas para que miren hacia arriba. TerrainData se guarda como asset antes de generar mapas de control para corregir su persistencia; el resultado cromático requiere nueva comprobación en Unity. Las demos 05/06 ya contienen una referencia ausente (GUID 780611a67e8e941a2b3aa96e5915a793), no reparada por esta revisión. El resto de la integración paisajística sigue pendiente.

**Revisión de relieve y puerto (10 septiembre 2026, tras autorización «ADELANTE», INC-188).** Se mantienen las escenas generadas anteriores. Los cinco caminos y el río se suavizan con dos pasadas Chaikin conservando extremos; el corredor vial combina las alturas con pesos continuos para evitar saltos al cambiar de segmento próximo. El valle fluvial pasa a transición lateral de 65 m y lecho de 1,8 m bajo agua; se comparte la curva entre cauce y superficie. Los caminos muestrean el Terrain real para limitar intersecciones con la malla discretizada. El macizo usa tres picos con aristas sobre una ladera común; todas las plataformas tienen transición elíptica. El puerto pasa a (270,4,-440), se orienta alrededor de su centro según las raíces Ship/Boat, sitúa el fondo de sus cascos en -0,8 m y excava sus huellas con salida abierta al sur. Si no identifica barcos, detiene la generación con error. La altura exacta de muelles requiere revisión visual. Las láminas orientales suspendidas se sustituyen por un arroyo apoyado en la ladera: las cascadas artísticas quedan pendientes. Más densidad del bosque, excluyendo caminos y cauce.

Validación de esta revisión: compilación aislada con referencias Unity correcta; cálculo independiente de las cinco curvas: 7,6 %, 3,8 %, 27,2 %, 1,9 % y 6,9 % de pendiente longitudinal máxima de diseño. El generador comprueba además terreno bajo barcos y alturas dentro de TerrainData antes de guardar. No se ha ejecutado esta revisión en el editor, ni verificado navegación o composición final; las pendientes de diseño no sustituyen un recorrido sobre colisiones reales. La validación narrativa se mantiene en cada generación (el último informe de la versión anterior registraba 0 errores y 37 advertencias).

**Revisión paisajística tras el trabajo de Claude (10 septiembre 2026, INC-188).** El generador se reparte en `Assets/Editor/EldoriaCodexBuilder.cs` y `Assets/Editor/EldoriaCodexBuilder.Paisaje.cs`. Conserva las variantes previas y el trabajo de integración de demos 09/05/06 y terrazas con castillo. La demo 10 ya no se importa. La variante comprobada de las 17:18 descartó el Terrain de la demo 09 por exceder el límite de tamaño existente: no se afirma que su suelo pintado esté conservado.

Cambios: lomas occidentales/meridionales y contrafuertes, transición más acotada de caminos y plataformas, islotes alargados con dos cabezas, siete capas de terreno (sotobosque y pradera añadidas), masas de arbolado mediante máscara irregular y claros, y árboles de alturas distintas. La caché de `FactorAltura` guardaba el factor de la primera altura solicitada; ahora guarda el inverso de la altura nativa y aplica cada altura objetivo. Se retiran parte de las hileras de árboles de las copias de demos 05/06; no se alteran sus prefabs ni las escenas originales. El faro tiene un basamento que abarca toda su huella. Las colecciones y cotas temporales se reinician en cada generación.

El arroyo se prepara antes del terreno y se excava; sus cotas no crecen aguas abajo. El río principal comparte cotas con el lecho, en lugar de situar el agua a 15 cm del fondo. Las comprobaciones miden profundidad central cada 2 m y registran puntos secos. Se añaden tres capturas automáticas (mapa, puerto y valle), aislando la escena mediante su máscara de render. Para esas capturas se clona temporalmente la calidad URP y se amplía la distancia de sombras; se restaura siempre la configuración original, sin guardar cambios en los assets de calidad.

Validación: compilación aislada correcta y generación real de `EldoriaCodex_20260910_171848`; capturas revisadas. La validación narrativa de esa variante dio **0 errores y 37 advertencias**. La inspección detectó un borde rectangular en el mar por el cálculo de profundidad del shader reutilizado en cámara ortográfica. La segunda pasada incorpora `EldoriaAgua.shader`, que utiliza batimetría del Terrain independiente de la cámara, y amplía las composiciones de los pueblos inicial/pesquero/cascada cuando no contienen Terrain propio. Esa segunda pasada está pendiente de generar y revisar. La maqueta sigue sin WorldBootstrap, anclas, NPCs narrativos ni NavMesh; no se declara terminada ni validada como mundo jugable.

La inicialización del editor solo genera automáticamente si existe una solicitud explícita en `Temp/EldoriaPaisaje-pendiente.txt`; consume el archivo antes de ejecutar, espera a salir de Play/compilación y guarda errores en `Temp/EldoriaPaisaje-error.txt`. Sin solicitud no realiza acciones. El menú habitual sigue disponible para nuevas variantes.

**Revisión urbana tras la prueba de Raúl (10 septiembre 2026 — INC-188).** Se parte de los archivos modificados por Claude hasta la variante de las 22:00, con copia de seguridad en `_CodexBackups/Eldoria-urbanismo-20260910`. Se añade `EldoriaCodexBuilder.Urbanismo.cs`. Cada ejecución sigue creando una escena separada: no sobrescribe MainWorld ni las variantes anteriores.

- Viviendas elegidas por identificador comprobado, en lugar de usar exclusivamente el tamaño de un prefab (el selector anterior aceptaba cercados y accesorios como casas). Escala nativa de viviendas conservada.
- Puerto y pueblo vecino pasan de composiciones de demo/accesorios a barrios propios con plazas. Embarcadero con colisión, pilotes y rampa. Castillo reducido y recolocado, plaza frontal despejada y barrio abierto.
- Las rasantes se aplican al heightmap después de transferir alturas de demo. Parcelas de edificios horizontales con transición; comprobaciones de apoyo en nueve puntos y pendientes reales del ascenso cada dos metros. Objetivo de pendiente longitudinal: 25 % o menos. Esto no acredita NavMesh, accesos interiores ni recorrido en Play.
- Se conservan las texturas del Terrain de demo 09 mediante copias de TerrainLayer con modulación de albedo y transición irregular. Se retiran los bloques decorativos de suelo de esa copia, se reduce la densidad global de flores y se rompen hileras exteriores de árboles. Los árboles independientes se apoyan por su base visible y se excluyen de taludes fuertes y parcelas.
- Cobertura del guión, huecos y ubicaciones propuestas documentados en GDD, apartado «Cobertura espacial de la maqueta Eldoria». No se modifica canon ni se conectan eventos por medio de estas reservas.

La generación produce ocho capturas e Informe.txt con apoyos, pendientes, agua y resultado del validador narrativo. Estado de la última comprobación y enlace de variante: TRACKER, INC-188.

**Comprobación del 11 septiembre 2026 — INC-188.** Variante `EldoriaCodex_20260911_095304`, generada en Unity y revisada mediante capturas. El generador se divide en el archivo principal y los parciales `.Paisaje.cs`, `.Urbanismo.cs` y `.Espacios.cs`. Se conserva también la revisión posterior de Claude que retira Ground/Hill de la demo y amplía la comprobación de pendientes a todos los caminos.

- Cuatro puentes con tablero de 4 m, barandas de aproximadamente 1 m y un MeshCollider continuo. Longitudes 23/38,5/29/29 m según el cruce; extremos a 0,025 m sobre el terreno. La búsqueda de orilla usa distancia al cauce: comparar con la cota de agua del centro alargaba indebidamente el puente del arroyo inclinado.
- Diez caminos muestreados cada 2 m sobre Terrain y colisión de tablero: máximo longitudinal 22,3 %. Empalmes del regreso del Golem proyectados sobre las curvas suavizadas para evitar discontinuidades. 68 parcelas/explanadas sin avisos en la comprobación; los edificios se verifican en nueve muestras. Calles secundarias excluidas si atraviesan otras casas o su desnivel medio supera 20 %; no cuentan como recorrido del jugador validado.
- Treinta viviendas adicionales repartidas entre montaña (16), puerto (8) y vecino (6), además de sus conjuntos base. Claros propuestos para Estela, Golem y Demonio 1, huertas y enseres. Monolito pétreo dentro de ruinas sustituye el cristal rosa; no incorpora Guardián ni eventos.
- `EldoriaMainWorldAgua.shader` es una copia específica del shader del agua utilizado por MainWorld: conserva sus propiedades visuales y corrige profundidad de ojo en perspectiva/ortográfica. Solo las copias de material de Eldoria usan este shader. El rectángulo de profundidad de las capturas anteriores ya no aparece.

Compilación aislada correcta; validación Interactive/Grafo: 0 errores, 37 advertencias. Pendientes prueba real en Play, accesos secundarios e interiores, navegación y acabado artístico. Los porcentajes geométricos no certifican por sí solos que el jugador pueda recorrer toda la isla.

**Revisión de obstáculos del 11 septiembre — INC-188.** `VerificarPasosLibres` consulta una cápsula de 0,70 × 1,80 m cada 0,5 m en los diez ejes principales. Filtra disparadores y colisionadores de otras escenas; informa consultas saturadas y obstáculos únicos. Es una prueba geométrica de gálibo, no una ejecución de Invector: no certifica la anchura completa de la calzada, árboles incorporados al Terrain, calles secundarias ni superficies sin collider. La primera ejecución encontró nueve colisionadores: piezas de dos viviendas y del santuario. Se apartan las casas de puerto/Reino, se ensancha la entrada a las ruinas y la senda termina antes del monolito. Añadidos faroles y cuatro conjuntos de mesas/asientos en bordes de plazas. Resultado de la variante final en TRACKER, INC-188.

**Continuación sobre la revisión 24 de Claude (11 septiembre, tarde — INC-188).** Copia previa de los generadores en `_CodexBackups/Eldoria-revision24-20260911`. Se conservan islas de jungla/ruinas, archipiélago, ruta marítima propuesta, vegetación por zonas y granjas. Las notas de ubicación anteriores quedan sustituidas por la cobertura espacial actualizada de GDD.

Muralla maciza `Wall02` elegida con el catálogo visual de assets, escala longitudinal calculada en el eje local del prefab (la AABB girada elegía un eje equivocado). Se omiten tramos/torres que invaden el camino principal; las calles secundarias no perforan el recinto. Se mantiene la retirada de la verja gigante ya preparada por Claude. Parcelas de viviendas recuperan prioridad dentro de su huella frente a mezclas de plazas y huertos. Las casas complementarias reservan holgura respecto a caminos usando su extensión visible. La casa occidental y su huerto de las granjas se separan del corredor de subida; las cercas respetan el paso. En la primera generación de esta continuación (`150107`) desaparecen los cuatro avisos geométricos y cinco obstáculos de `135013`; la revisión visual motiva acotar los huecos de muralla únicamente a las vías principales. Última variante y validación en TRACKER.


**Suelos urbanos del 17 septiembre — INC-188.** Se parte de los generadores actuales (modificados hasta el 12 septiembre), con copia previa en `_CodexBackups/Eldoria-suelos-20260917`. Los ocho colores procedurales se conservan para el paisaje; cinco capas adicionales usan las texturas originales Fantasy Kingdom `Ground03_D`, `Terrain03_D`, `Terrain04_D`, `Ground05_D` y `Terrain01_D`, sin modificar assets originales. Las capas de la demo 09 se siguen incorporando después, conservando el acabado del pueblo central.

`MezclarSuelo` realiza mezclas convexas, sustituyendo la combinación de tierra/piedra que podía producir pesos negativos de prado. El Reino diferencia calles pétreas y plazas de baldosa clara con marco; puerto y vecino combinan accesos adoquinados, patios de tierra y plazas de piedra. Las huertas, incluidos nombres «Huerta», se consideran naturales. Los accesos secundarios pueden conectar con una plaza cercana si pasan el filtro de desnivel medio y no atraviesan otras viviendas; siguen sin equivaler a un recorrido jugable certificado.

Parterres del Reino: candidatos separados de solares y caminos; se descartan pendientes fuertes después de construir el Terrain. Solo los cuatro aceptados reciben árbol, flores, borde bajo y pintura de hierba. Se evita dejar manchas de pintura en candidatos descartados. Generación final `EldoriaCodex_20260917_131554`: 94 parcelas/explanadas sin avisos, 5932 muestras de gálibo sin obstáculos ni saturaciones. Validación Interactive/Grafo: 0 errores y 40 advertencias en el estado actual del proyecto. No se modifican sistemas narrativos; la cifra previa de 37 correspondía al estado anterior. Compilación aislada y revisión de capturas completadas. Pendientes prueba en Play y acabado profesional de relieve, composición y espacios secundarios.
