...
public static partial class EldoriaCodexBuilder
{
    ...
    {
        var lienzo=AssetDatabase.LoadAssetAtPath<GameObject>(Pack+"Main Structures/Wall/Wall01.prefab");
        var torre=AssetDatabase.LoadAssetAtPath<GameObject>(Pack+"Main Structures/Wall/Tower01.prefab");
        if(lienzo==null||torre==null){informe.AppendLine("Muralla del Reino omitida: Wall01/Tower01 no encontrados en Main Structures/Wall.");return;}
        ...
        for(int i=0;i<MurallaTrazado.Length;i++)
        {
            ...
            for(int k=0;k<n;k++)
            {
                ...
                float sx=lado>.1f?(L/n+.15f)/lado:1, sy=bb.size.y>.1f?alturaMuro/bb.size.y:1;
                bool largoEnX=bb.size.x>=bb.size.z;
        ...
        informe.AppendLine($"Muralla del Reino: {tramos} lienzos (Wall01 a {alturaMuro} m), {MurallaTrazado.Length} torres (Tower01 a {alturaTorre} m), puerta al sureste (Castle01_b01). Revisar que los lienzos casen en las esquinas y que la puerta mire a la calle.");
    }
    ...
    static void ConstruirGranjaCascada(Zona zona)
    {
        ...
        var c=zona.centro;
        PiezaUrbana(zona.grupo,Pack+"Building Combination/"+Viviendas[3]+".prefab","Casa de labranza 1 — campo de Erika",c+new Vector3(-24,0,10),Vector3.right);
        ...
        Plaza(c+new Vector3(-24,0,-8),new Vector2(12,10),"Huerto de la casa 1");
        Plaza(c+new Vector3(18,0,8),new Vector2(14,10),"Huerto junto al arroyo");
