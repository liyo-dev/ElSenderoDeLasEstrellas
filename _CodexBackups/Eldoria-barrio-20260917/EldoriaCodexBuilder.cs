...
public static partial class EldoriaCodexBuilder
{
    ...
    static void ConstruirTerreno()
    {
        ...
        var capas=new TerrainLayer[colores.Length+5];
        ...
        // Capas urbanas independientes: el pueblo central conserva las capas de su demo.
        string[] texturasUrbanas={"Ground03_D", "Terrain03_D", "Terrain04_D", "Ground05_D", "Terrain01_D"};
        for(int i=0;i<texturasUrbanas.Length;i++)
        {
            var tex=AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Art/World/Fantasy_Kingdom_Pack/Textures/"+texturasUrbanas[i]+".tga");
            if(tex==null)throw new InvalidOperationException("Falta textura urbana: "+texturasUrbanas[i]);
            capas[8+i]=new TerrainLayer{diffuseTexture=tex,tileSize=new Vector2(6,6),smoothness=0,metallic=0,
                diffuseRemapMin=Vector4.zero,diffuseRemapMax=new Vector4(.78f,.80f,.76f,0)};
            AssetDatabase.CreateAsset(capas[8+i],carpeta+"/SueloUrbano"+i+".terrainlayer");
        }
        datos.terrainLayers=capas;var pesos=new float[512,512,capas.Length];
