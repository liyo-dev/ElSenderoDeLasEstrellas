...
public static partial class EldoriaCodexBuilder
{
    ...
    static void PrepararEspacios()
    {
        ...
        // Parterres en huecos reales del Reino, separados de parcelas, calzadas y murallas.
        for(float z=251;z<=331;z+=12)for(float x=-75;x<=75;x+=12)
        {
            if(DistanciaCaminos(x,z)<7)continue;
            bool libre=true;
            foreach(var solar in solares)if(PesoSolar(solar,x,z,5)>.01f){libre=false;break;}
            if(!libre)continue;
            foreach(var jardin in jardinesUrbanos)if(Vector2.Distance(new Vector2(x,z),new Vector2(jardin.x,jardin.z))<15){libre=false;break;}
            if(!libre)continue;
            jardinesUrbanos.Add(new Vector3(x,0,z));
            despejes.Add(new Vector3(x,z,6));
        }
